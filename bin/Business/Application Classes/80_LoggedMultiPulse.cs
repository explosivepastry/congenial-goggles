...
public class LoggedMultiPulse : MonnitApplicationBase, ISensorAttribute
{
  private SensorAttribute _TransformValue;
  private SensorAttribute _Label;

  public static long MonnitApplicationID => 80 /*0x50*/;

  public static string ApplicationName => "Logged Multi Pulse Counter";

  public static eApplicationProfileType ProfileType => eApplicationProfileType.Interval;

  public override string ChartType => "Line";

  public override long ApplicationID => LoggedMultiPulse.MonnitApplicationID;
  ...
  {
    return (MonnitApplicationBase) LoggedMultiPulse.Deserialize(version, serialized);
  }
  ...
  public static LoggedMultiPulse Deserialize(string version, string serialized)
  {
    LoggedMultiPulse loggedMultiPulse = new LoggedMultiPulse();
    if (string.IsNullOrEmpty(serialized))
    {
      loggedMultiPulse.Count1 = 0;
      loggedMultiPulse.Count2 = 0;
    }
    else
    {
      string[] strArray = serialized.Split(',');
      loggedMultiPulse.Count1 = strArray[0].ToInt();
      if (strArray.Length == 4)
      {
        loggedMultiPulse.Count2 = strArray[1].ToInt();
        loggedMultiPulse.Count3 = strArray[2].ToInt();
        loggedMultiPulse.Count4 = strArray[3].ToInt();
      }
      else
      {
        loggedMultiPulse.Count2 = loggedMultiPulse.Count1;
        loggedMultiPulse.Count3 = loggedMultiPulse.Count1;
        loggedMultiPulse.Count4 = loggedMultiPulse.Count1;
      }
    }
    return loggedMultiPulse;
  }

  public static LoggedMultiPulse Create(byte[] sdm, int startIndex)
  {
    return new LoggedMultiPulse()
    {
  ...
  public new static Notification NotificationByApplicationID(Sensor sensor)
  {
    return new Notification()
    {
      ...
      ApplicationID = LoggedMultiPulse.MonnitApplicationID,
      SnoozeDuration = 60,
    ...
    NameValueCollection viewData)
  {
    if (!string.IsNullOrEmpty(collection["label"]))
      LoggedMultiPulse.SetLabel(sensor.SensorID, collection["label"]);
    if (string.IsNullOrEmpty(collection["transformValue"]))
      return;
    LoggedMultiPulse.SetTransform(sensor.SensorID, collection["transformValue"].ToDouble());
  }
    ...
    NameValueCollection viewData)
  {
    ...
    int result = 0;
    if (string.IsNullOrWhiteSpace(collection["debounce"]) || !int.TryParse(collection["debounce"], out result))
      return;
    if (collection["debounce"].ToInt() < 3)
      collection["debounce"] = "3";
    if (collection["debounce"].ToInt() > 30)
      collection["debounce"] = "30";
    sensor.Calibration4 = (long) (collection["debounce"].ToInt() * 1000);
  }
  ...
  public new static List<byte[]> ActionControlCommand(Sensor sensor, string version)
  {
    ...
    {
      LoggedMultiPulseBase.getCalibrationValue(sensor.SensorID),
  ...
  public static bool operator ==(LoggedMultiPulse left, LoggedMultiPulse right)
  {
  ...
  {
    return obj is LoggedMultiPulse && this.Equals((MonnitApplicationBase) (obj as LoggedMultiPulse));
  }

  public override bool Equals(MonnitApplicationBase right)
  {
    return right is LoggedMultiPulse && this.Count1 == (right as LoggedMultiPulse).Count1;
  }
