...
// Type: Monnit.CurrentZeroToFiveAmp
...
public class CurrentZeroToFiveAmp : MonnitApplicationBase, ISensorAttribute
{
  private SensorAttribute _Label;
  private SensorAttribute _VoltValue;
  private SensorAttribute _ShowFullDataValue;

  public static long MonnitApplicationID => CurrentZeroToFiveAmpBase.MonnitApplicationID;

  public static string ApplicationName => "Current Meter - 5A";

  public static eApplicationProfileType ProfileType => CurrentBase.ProfileType;

  public override string ChartType => "Line";

  public override long ApplicationID => CurrentZeroToFiveAmp.MonnitApplicationID;
  ...
  public override List<string> GetPlotLabels(long sensorID)
  {
    ...
    {
      CurrentZeroToFiveAmp.GetLabel(sensorID),
      "AvgCurrent",
      "MaxCurrent",
      "MinCurrent"
    });
  ...
  public new static NotificationScaleInfoModel GetScalingInfo(Sensor sens)
  {
    ...
    CurrentZeroToFiveAmp currentZeroToFiveAmp = new CurrentZeroToFiveAmp();
    currentZeroToFiveAmp.SetSensorAttributes(sens.SensorID);
    scalingInfo.Label = currentZeroToFiveAmp.Label.Value.ToStringSafe();
    scalingInfo.profileLow = 0.0;
    scalingInfo.profileHigh = !(currentZeroToFiveAmp.Label.Value == "Wh") ? (!(currentZeroToFiveAmp.Label.Value == "kWh") ? 20.0 : currentZeroToFiveAmp.VoltValue.Value.ToDouble() * 20.0 / 1000.0) : currentZeroToFiveAmp.VoltValue.Value.ToDouble() * 20.0;
  ...
  {
    return (MonnitApplicationBase) CurrentZeroToFiveAmp.Deserialize(version, serialized);
  }
  ...
  {
    return $"\"{((CurrentZeroToFiveAmp) app).AmpHours}\",\"{((CurrentZeroToFiveAmp) app).AvgCurrent}\",\"{((CurrentZeroToFiveAmp) app).MaxCurrent}\",\"{((CurrentZeroToFiveAmp) app).MinCurrent}\"";
  }

  public static CurrentZeroToFiveAmp Deserialize(string version, string serialized)
  {
    CurrentZeroToFiveAmp currentZeroToFiveAmp = new CurrentZeroToFiveAmp();
    if (string.IsNullOrEmpty(serialized))
    {
      currentZeroToFiveAmp.AmpHours = 0.0;
      currentZeroToFiveAmp.AvgCurrent = 0.0;
      currentZeroToFiveAmp.MaxCurrent = 0.0;
      currentZeroToFiveAmp.MinCurrent = 0.0;
    }
    else
    {
      string[] strArray = serialized.Split(',');
      currentZeroToFiveAmp.AmpHours = strArray[0].ToDouble();
      if (strArray.Length == 4)
      {
        currentZeroToFiveAmp.AvgCurrent = strArray[1].ToDouble();
        currentZeroToFiveAmp.MaxCurrent = strArray[2].ToDouble();
        currentZeroToFiveAmp.MinCurrent = strArray[3].ToDouble();
      }
      else
      {
        currentZeroToFiveAmp.AvgCurrent = strArray[0].ToDouble();
        currentZeroToFiveAmp.MaxCurrent = strArray[0].ToDouble();
        currentZeroToFiveAmp.MinCurrent = strArray[0].ToDouble();
      }
    }
    return currentZeroToFiveAmp;
  }

  public static CurrentZeroToFiveAmp Create(byte[] sdm, int startIndex)
  {
    return new CurrentZeroToFiveAmp()
    {
      AmpHours = BitConverter.ToUInt64(sdm, startIndex).ToDouble() / 1000.0,
      AvgCurrent = BitConverter.ToUInt16(sdm, startIndex + 8).ToDouble() / 1000.0,
      MaxCurrent = BitConverter.ToUInt16(sdm, startIndex + 10).ToDouble() / 1000.0,
      MinCurrent = BitConverter.ToUInt16(sdm, startIndex + 12).ToDouble() / 1000.0
    };
  }
  ...
  public new static Notification NotificationByApplicationID(Sensor sensor)
  {
    return new Notification()
    {
      ...
      ApplicationID = CurrentZeroToFiveAmp.MonnitApplicationID,
    ...
    NameValueCollection viewData)
  {
    if (!string.IsNullOrEmpty(collection["voltValue"]))
      CurrentZeroToFiveAmp.SetVoltValue(sensor.SensorID, collection["voltValue"].ToDouble());
    if (string.IsNullOrEmpty(collection["label"]))
      return;
    CurrentZeroToFiveAmp.SetLabel(sensor.SensorID, collection["label"]);
  }
    ...
    NameValueCollection viewData)
  {
    ...
    else
    {
      ...
      if (!string.IsNullOrEmpty(collection["Hysteresis_Manual"]))
      {
        ...
        {
          if (result < 0.0)
            CurrentZeroToFiveAmp.SetHystLower(sensor, (ushort) 0);
          else if (result > 5.0)
            CurrentZeroToFiveAmp.SetHystLower(sensor, Convert.ToUInt16(5000));
          else
            CurrentZeroToFiveAmp.SetHystLower(sensor, Convert.ToUInt16(result * 1000.0));
        }
        else
          sensor.Hysteresis = sensor.DefaultValue<long>("DefaultHysteresis");
      }
      ...
      if (!string.IsNullOrEmpty(collection["MaximumThreshold_Manual"]))
      {
        if (double.TryParse(collection["MaximumThreshold_Manual"], out result))
        {
          ...
          double num2 = (double) sensor.MinimumThreshold / 1000.0;
          if (num1 < 0.0)
            num1 = 0.0;
          if (num1 > 30.0)
            num1 = 30.0;
          if (num1 < num2)
            num1 = num2;
          sensor.MaximumThreshold = (num1 * 1000.0).ToLong();
        }
        else
          sensor.MaximumThreshold = sensor.DefaultValue<long>("DefaultMaximumThreshold");
      }
      ...
      if (!string.IsNullOrEmpty(collection["currentShift"]))
      {
        ...
        sensor.Calibration4 = num >= 0.0 ? (num <= (double) ushort.MaxValue ? (collection["currentShift"].ToDouble() * 1000.0).ToLong() : 65535000L) : 0L;
      }
      if (!string.IsNullOrEmpty(collection["Offset_Hidden"]))
        CurrentZeroToFiveAmp.SetHystThirdByte(sensor, Convert.ToSByte(collection["Offset_Hidden"].ToDouble() * 100.0));
      if (!string.IsNullOrEmpty(collection["Accum"]))
        CurrentZeroToFiveAmp.SetHystFourthByte(sensor, Convert.ToByte(collection["Accum"]));
      if (!string.IsNullOrEmpty(collection["FullNotiString"]))
      {
        ...
        CurrentZeroToFiveAmp.SetShowFullDataValue(sensor.SensorID, showFullData);
      }
    }
  }
  ...
  public static void CalibrateSensor(Sensor sensor, NameValueCollection collection)
  {
    ...
    if (!string.IsNullOrWhiteSpace(collection["observed"]))
    {
      ...
      num = collection["observed"].ToDouble() * 1000.0;
    ...
    if (!string.IsNullOrWhiteSpace(collection["actual"]))
    {
      ...
      num = collection["actual"].ToDouble() * 1000.0;
      string str = num.ToString();
  ...
  public new static List<byte[]> ActionControlCommand(Sensor sensor, string version)
  {
    ...
    numArrayList.Add(CurrentZeroToFiveAmpBase.CalibrateFrame(sensor.SensorID, acc, oldVal, newVal));
  ...
  {
    return sensor.Hysteresis == (long) uint.MaxValue ? "" : ((double) CurrentZeroToFiveAmp.GetHystLower(sensor) / 1000.0).ToString();
  }

  public static string MinThreshForUI(Sensor sensor)
  {
    return (sensor.MinimumThreshold.ToDouble() / 1000.0).ToString();
  }

  public static string MaxThreshForUI(Sensor sensor)
  {
    return (sensor.MaximumThreshold.ToDouble() / 1000.0).ToString();
  }

  public override int GetHashCode() => base.GetHashCode();

  public static bool operator ==(CurrentZeroToFiveAmp left, CurrentZeroToFiveAmp right)
  {
  ...
  {
    return obj is CurrentZeroToFiveAmp && this.Equals((MonnitApplicationBase) (obj as CurrentZeroToFiveAmp));
  }

  public override bool Equals(MonnitApplicationBase right)
  {
    return right is CurrentZeroToFiveAmp && this.AvgCurrent == (right as CurrentZeroToFiveAmp).AvgCurrent;
  }
