...
public class BatteryHealth : MonnitApplicationBase
{
  ...
  public static long MonnitApplicationID => 59;

  public static string ApplicationName => "Voltage Meter - 50 VDC";

  public static eApplicationProfileType ProfileType => eApplicationProfileType.Interval;

  public override string ChartType => "Line";

  public override long ApplicationID => BatteryHealth.MonnitApplicationID;
  ...
  public override List<AppDatum> Datums
  {
    get
    {
      ...
      {
        new AppDatum(eDatumType.UnscaledVoltage, "Volts", this.Voltage)
      });
    }
  }
  ...
  {
    return (MonnitApplicationBase) BatteryHealth.Deserialize(version, serialized);
  }

  public override string NotificationString => this.Voltage.ToString("#0.00") + " Volts";

  public override object PlotValue => (object) Math.Round(this.Voltage, 1);

  public static BatteryHealth Deserialize(string version, string serialized)
  {
  ...
  public static BatteryHealth Create(byte[] sdm, int startIndex)
  {
    return new BatteryHealth()
    {
      Voltage = BitConverter.ToUInt16(sdm, startIndex).ToDouble() / 1000.0
    };
  }
  ...
  public new static Notification NotificationByApplicationID(Sensor sensor)
  {
    return new Notification()
    {
      ...
      ApplicationID = BatteryHealth.MonnitApplicationID,
      SnoozeDuration = 60,
    ...
    NameValueCollection viewData)
  {
    ...
    batteryHealth.SetSensorAttributes(sensor.SensorID);
    double num1 = batteryHealth.LowValue.Value.ToDouble();
    double num2 = batteryHealth.HighValue.Value.ToDouble();
    if (!string.IsNullOrEmpty(collection["Hysteresis_Manual"]) || double.TryParse(collection["Hysteresis_Manual"], out result))
    {
      double num3 = Math.Abs((num2 - num1) / 50.0);
    ...
    if (!string.IsNullOrEmpty(collection["MinimumThreshold_Manual"]) || double.TryParse(collection["MinimumThreshold_Manual"], out result))
    {
      ...
      sensor.MinimumThreshold = x >= num1 ? (x <= num2 ? (x.LinearInterpolation(num1, 0.0, num2, 50.0) * 1000.0).ToLong() : (num2.LinearInterpolation(num1, 0.0, num2, 50.0) * 1000.0).ToLong()) : (num1.LinearInterpolation(num1, 0.0, num2, 50.0) * 1000.0).ToLong();
    }
    if (!string.IsNullOrEmpty(collection["MaximumThreshold_Manual"]) || double.TryParse(collection["MaximumThreshold_Manual"], out result))
    {
      ...
      sensor.MaximumThreshold = x >= num1 ? (x <= num2 ? (x.LinearInterpolation(num1, 0.0, num2, 50.0) * 1000.0).ToLong() : (num2.LinearInterpolation(num1, 0.0, num2, 50.0) * 1000.0).ToLong()) : (num1.LinearInterpolation(num1, 0.0, num2, 50.0) * 1000.0).ToLong();
    ...
    if (collection["showAdvCal"] == "on")
    {
      ...
      BatteryHealth.SetShowAdvCal(sensor.SensorID, "true");
    }
    else
    {
      viewData["showAdvCal"] = "false";
      BatteryHealth.SetShowAdvCal(sensor.SensorID, "false");
    }
  }
  ...
  public new static List<byte[]> ActionControlCommand(Sensor sensor, string version)
  {
    ...
    {
      BatteryHealthBase.CalibrateFrame(sensor.SensorID, (double) sensor.Calibration1 / 1000.0),
  ...
  public static void CalibrateSensor(Sensor sensor, NameValueCollection collection)
  {
    if (!(new Version(sensor.FirmwareVersion) > new Version("2.0.0.0")) && sensor.SensorTypeID != 4L)
      return;
    int num1 = (collection["observed"].ToDouble().LinearInterpolation(BatteryHealth.GetLowValue(sensor.SensorID), 0.0, BatteryHealth.GetHighValue(sensor.SensorID), 55.0) * 1000.0).ToInt();
    int num2 = (collection["actual"].ToDouble().LinearInterpolation(BatteryHealth.GetLowValue(sensor.SensorID), 0.0, BatteryHealth.GetHighValue(sensor.SensorID), 55.0) * 1000.0).ToInt();
    if (num1 > 0 && num2 > 0)
    {
    ...
    NameValueCollection viewData)
  {
    if (!string.IsNullOrEmpty(collection["lowValue"]))
      BatteryHealth.SetLowValue(sensor.SensorID, collection["lowValue"].ToDouble());
    if (!string.IsNullOrEmpty(collection["highValue"]))
      BatteryHealth.SetHighValue(sensor.SensorID, collection["highValue"].ToDouble());
    if (string.IsNullOrEmpty(collection["label"]))
      return;
    BatteryHealth.SetLabel(sensor.SensorID, collection["label"]);
  }
  ...
  public static string HystForUI(Sensor sensor)
  {
    ...
    batteryHealth.SetSensorAttributes(sensor.SensorID);
    double num1 = batteryHealth.LowValue.Value.ToDouble();
    double num2 = Math.Abs((batteryHealth.HighValue.Value.ToDouble() - num1) / 50.0);
  ...
  public static string MinThreshForUI(Sensor sensor)
  {
    BatteryHealth batteryHealth = new BatteryHealth();
    batteryHealth.SetSensorAttributes(sensor.SensorID);
    double lowY = batteryHealth.LowValue.Value.ToDouble();
    double highY = batteryHealth.HighValue.Value.ToDouble();
    return sensor.MinimumThreshold == (long) uint.MaxValue ? "" : (sensor.MinimumThreshold.ToDouble() / 1000.0).LinearInterpolation(0.0, lowY, 50.0, highY).ToString();
  }
  ...
  public static string MaxThreshForUI(Sensor sensor)
  {
    ...
    double lowY = batteryHealth.LowValue.Value.ToDouble();
    double highY = batteryHealth.HighValue.Value.ToDouble();
    return sensor.MaximumThreshold == (long) uint.MaxValue ? "" : ((double) sensor.MaximumThreshold / 1000.0).LinearInterpolation(0.0, lowY, 50.0, highY).ToString();
  }

  public override int GetHashCode() => base.GetHashCode();

  public static bool operator ==(BatteryHealth left, BatteryHealth right)
  {
  ...
  {
    return obj is BatteryHealth && this.Equals((MonnitApplicationBase) (obj as BatteryHealth));
  }

  public override bool Equals(MonnitApplicationBase right)
  {
    return right is BatteryHealth && this.Voltage == (right as BatteryHealth).Voltage;
  }
  ...
  public new static NotificationScaleInfoModel GetScalingInfo(Sensor sens)
  {
    ...
    BatteryHealth batteryHealth = new BatteryHealth();
    batteryHealth.SetSensorAttributes(sens.SensorID);
    scalingInfo.Label = batteryHealth.Label.Value.ToStringSafe();
    scalingInfo.profileLow = batteryHealth.LowValue.Value.ToDouble();
    scalingInfo.profileHigh = batteryHealth.HighValue.Value.ToDouble();
