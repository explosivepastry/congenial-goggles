...
// Type: Monnit.CurrentZeroToOneFiftyAmp
...
public class CurrentZeroToOneFiftyAmp : MonnitApplicationBase, ISensorAttribute
{
  private SensorAttribute _Label;
  private SensorAttribute _VoltValue;
  private SensorAttribute _ShowFullDataValue;

  public static long MonnitApplicationID => CurrentZeroToOneFiftyAmpBase.MonnitApplicationID;

  public static string ApplicationName => "Current Meter - 150A";

  public static eApplicationProfileType ProfileType => CurrentBase.ProfileType;

  public override string ChartType => "Line";

  public override long ApplicationID => CurrentZeroToOneFiftyAmp.MonnitApplicationID;
  ...
  public override List<string> GetPlotLabels(long sensorID)
  {
    ...
    {
      CurrentZeroToOneFiftyAmp.GetLabel(sensorID),
      "AvgCurrent",
      "MaxCurrent",
      "MinCurrent"
    });
  ...
  public new static NotificationScaleInfoModel GetScalingInfo(Sensor sens)
  {
    ...
    scalingInfo.profileHigh = !(currentZeroToTwentyAmp.Label.Value == "Wh") ? (!(currentZeroToTwentyAmp.Label.Value == "kWh") ? 150.0 : currentZeroToTwentyAmp.VoltValue.Value.ToDouble() * 150.0 / 1000.0) : currentZeroToTwentyAmp.VoltValue.Value.ToDouble() * 150.0;
    scalingInfo.baseLow = 0.0;
    scalingInfo.baseHigh = 150.0;
    return scalingInfo;
  }
  ...
  {
    return (MonnitApplicationBase) CurrentZeroToOneFiftyAmp.Deserialize(version, serialized);
  }
  ...
  {
    return $"\"{((CurrentZeroToOneFiftyAmp) app).AmpHours}\",\"{((CurrentZeroToOneFiftyAmp) app).AvgCurrent}\",\"{((CurrentZeroToOneFiftyAmp) app).MaxCurrent}\",\"{((CurrentZeroToOneFiftyAmp) app).MinCurrent}\"";
  }

  public static CurrentZeroToOneFiftyAmp Deserialize(string version, string serialized)
  {
    CurrentZeroToOneFiftyAmp zeroToOneFiftyAmp = new CurrentZeroToOneFiftyAmp();
    if (string.IsNullOrEmpty(serialized))
    {
      zeroToOneFiftyAmp.AmpHours = 0.0;
      zeroToOneFiftyAmp.AvgCurrent = 0.0;
      zeroToOneFiftyAmp.MaxCurrent = 0.0;
      zeroToOneFiftyAmp.MinCurrent = 0.0;
    }
    else
    {
      string[] strArray = serialized.Split(',');
      zeroToOneFiftyAmp.AmpHours = strArray[0].ToDouble();
      if (strArray.Length == 4)
      {
        zeroToOneFiftyAmp.AvgCurrent = strArray[1].ToDouble();
        zeroToOneFiftyAmp.MaxCurrent = strArray[2].ToDouble();
        zeroToOneFiftyAmp.MinCurrent = strArray[3].ToDouble();
      }
      else
      {
        zeroToOneFiftyAmp.AvgCurrent = strArray[0].ToDouble();
        zeroToOneFiftyAmp.MaxCurrent = strArray[0].ToDouble();
        zeroToOneFiftyAmp.MinCurrent = strArray[0].ToDouble();
      }
    }
    return zeroToOneFiftyAmp;
  }

  public static CurrentZeroToOneFiftyAmp Create(byte[] sdm, int startIndex)
  {
    return new CurrentZeroToOneFiftyAmp()
    {
  ...
  public new static Notification NotificationByApplicationID(Sensor sensor)
  {
    return new Notification()
    {
      ...
      ApplicationID = CurrentZeroToOneFiftyAmp.MonnitApplicationID,
    ...
    NameValueCollection viewData)
  {
    if (!string.IsNullOrEmpty(collection["voltValue"]))
      CurrentZeroToOneFiftyAmp.SetVoltValue(sensor.SensorID, collection["voltValue"].ToDouble());
    if (string.IsNullOrEmpty(collection["label"]))
      return;
    CurrentZeroToOneFiftyAmp.SetLabel(sensor.SensorID, collection["label"]);
  }
    ...
    NameValueCollection viewData)
  {
    ...
    else
    {
      ...
      if (!string.IsNullOrEmpty(collection["Hysteresis_Manual"]))
      {
        ...
        {
          if (result < 0.0)
            CurrentZeroToOneFiftyAmp.SetHystLower(sensor, (ushort) 0);
          else if (result > 5.0)
            CurrentZeroToOneFiftyAmp.SetHystLower(sensor, Convert.ToUInt16(500));
          else
            CurrentZeroToOneFiftyAmp.SetHystLower(sensor, Convert.ToUInt16(result * 100.0));
        }
        else
          sensor.Hysteresis = sensor.DefaultValue<long>("DefaultHysteresis");
      }
      ...
      {
        CurrentZeroToOneFiftyAmp.SetHystThirdByte(sensor, Convert.ToSByte(collection["Offset_Hidden"].ToDouble() * 100.0));
        CurrentZeroToOneFiftyAmp.SetHystFourthByte(sensor, Convert.ToByte(collection["Accum"]));
      }
      if (!string.IsNullOrEmpty(collection["FullNotiString"]))
      {
        ...
        CurrentZeroToOneFiftyAmp.SetShowFullDataValue(sensor.SensorID, showFullData);
      }
    }
  }
  ...
  {
    CurrentZeroToOneFiftyAmp.CalibrateSensor(sens, new NameValueCollection()
    {
      {
        "acc",
  ...
  public new static List<byte[]> ActionControlCommand(Sensor sensor, string version)
  {
    ...
    {
      numArrayList.Add(CurrentZeroToOneFiftyAmpBase.CalibrateFrame(sensor.SensorID, acc, oldVal, newVal, SensorAttribute.GetQueueID(sensor.SensorID), new long?(sensor.CableID)));
      numArrayList.Add(sensor.ReadProfileConfig(29));
    }
    return numArrayList;
  }
  ...
  {
    return sensor.Hysteresis == (long) uint.MaxValue ? "" : ((double) CurrentZeroToOneFiftyAmp.GetHystLower(sensor) / 100.0).ToString();
  }
  ...
  public static bool operator ==(CurrentZeroToOneFiftyAmp left, CurrentZeroToOneFiftyAmp right)
  {
  ...
  {
    return obj is CurrentZeroToOneFiftyAmp && this.Equals((MonnitApplicationBase) (obj as CurrentZeroToOneFiftyAmp));
  }

  public override bool Equals(MonnitApplicationBase right)
  {
    return right is CurrentZeroToOneFiftyAmp && this.AvgCurrent == (right as CurrentZeroToOneFiftyAmp).AvgCurrent;
  }
  ...
  public static long DefaultMaxThreshold => 15000;
