...
// Type: Monnit.ThreePhase500AmpMeter
...
public class ThreePhase500AmpMeter : MonnitApplicationBase, ISensorAttribute
{
  ...
  public static long MonnitApplicationID => 129;

  public static string ApplicationName => "Three Phase 500 Amp Meter";

  public static eApplicationProfileType ProfileType => eApplicationProfileType.Interval;

  public override string ChartType => "Line";

  public override long ApplicationID => ThreePhase500AmpMeter.MonnitApplicationID;
    ...
    get
    {
      if (this._WattHours <= 0.0)
      {
        switch (this.Label.Value.ToLower())
        {
          case "mj":
          case "kwh":
            this._WattHours = !this.UseEstimatedPower.Value.ToBool() ? Math.Round(this.TotalCurrentAccumulation * this.VoltValue.Value.ToDouble(), 1) : ThreePhase500AmpMeter.CalculatePowerEstimate(this.VoltValue.Value.ToDouble(), this.GetPhaseReading(this.SelectedCurrentReading.Value.ToInt()), this.PowerFactor.Value.ToDouble(), this.GetHoursForCurrentReading());
            break;
          default:
            this._WattHours = 0.0;
            break;
  ...
  public static ThreePhase500AmpMeter Create(byte[] sdm, int startIndex)
  {
    ThreePhase500AmpMeter phase500AmpMeter = new ThreePhase500AmpMeter();
    try
    {
      phase500AmpMeter.Phase1Average = BitConverter.ToUInt16(sdm, startIndex).ToDouble() / 100.0;
      phase500AmpMeter.Phase1Max = (int) sdm[startIndex + 2] * 3;
      phase500AmpMeter.Phase1Min = (int) sdm[startIndex + 3] * 3;
      phase500AmpMeter.Phase1Duty = (int) sdm[startIndex + 4];
      phase500AmpMeter.Phase2Average = BitConverter.ToUInt16(sdm, startIndex + 5).ToDouble() / 100.0;
      phase500AmpMeter.Phase2Max = (int) sdm[startIndex + 7] * 3;
      phase500AmpMeter.Phase2Min = (int) sdm[startIndex + 8] * 3;
      phase500AmpMeter.Phase2Duty = (int) sdm[startIndex + 9];
      phase500AmpMeter.Phase3Average = BitConverter.ToUInt16(sdm, startIndex + 10).ToDouble() / 100.0;
      phase500AmpMeter.Phase3Max = (int) sdm[startIndex + 12] * 3;
      phase500AmpMeter.Phase3Min = (int) sdm[startIndex + 13] * 3;
      phase500AmpMeter.Phase3Duty = (int) sdm[startIndex + 14];
      phase500AmpMeter.TotalCurrentAccumulation = BitConverter.ToUInt32(sdm, startIndex + 15).ToDouble() / 10.0;
    }
    catch
    {
      phase500AmpMeter.Phase1Average = 0.0;
      phase500AmpMeter.Phase1Max = 0;
      phase500AmpMeter.Phase1Min = 0;
      phase500AmpMeter.Phase1Duty = 0;
      phase500AmpMeter.Phase2Average = 0.0;
      phase500AmpMeter.Phase2Max = 0;
      phase500AmpMeter.Phase2Min = 0;
      phase500AmpMeter.Phase2Duty = 0;
      phase500AmpMeter.Phase3Average = 0.0;
      phase500AmpMeter.Phase3Max = 0;
      phase500AmpMeter.Phase3Min = 0;
      phase500AmpMeter.Phase3Duty = 0;
      phase500AmpMeter.TotalCurrentAccumulation = 0.0;
    }
    return phase500AmpMeter;
  }
  ...
  {
    return (MonnitApplicationBase) ThreePhase500AmpMeter.Deserialize(version, serialized);
  }
  ...
  public static ThreePhase500AmpMeter Deserialize(string version, string serialized)
  {
    ThreePhase500AmpMeter phase500AmpMeter = new ThreePhase500AmpMeter();
    if (string.IsNullOrEmpty(serialized))
    {
      ...
      phase500AmpMeter.Phase3Duty = 0;
      phase500AmpMeter.TotalCurrentAccumulation = 0.0;
      phase500AmpMeter.WattHours = 0.0;
    }
    else
    {
      ...
      try
      {
        phase500AmpMeter.Phase1Average = strArray[0].ToDouble();
        if (strArray.Length > 1)
        {
          phase500AmpMeter.Phase1Max = strArray[1].ToInt();
          phase500AmpMeter.Phase1Min = strArray[2].ToInt();
          phase500AmpMeter.Phase1Duty = strArray[3].ToInt();
          phase500AmpMeter.Phase2Average = strArray[4].ToDouble();
          phase500AmpMeter.Phase2Max = strArray[5].ToInt();
          phase500AmpMeter.Phase2Min = strArray[6].ToInt();
          phase500AmpMeter.Phase2Duty = strArray[7].ToInt();
          phase500AmpMeter.Phase3Average = strArray[8].ToDouble();
          phase500AmpMeter.Phase3Max = strArray[9].ToInt();
          phase500AmpMeter.Phase3Min = strArray[10].ToInt();
          phase500AmpMeter.Phase3Duty = strArray[11].ToInt();
          phase500AmpMeter.TotalCurrentAccumulation = strArray[12].ToDouble();
          if (strArray.Length > 13)
            phase500AmpMeter.WattHours = !double.IsNaN(strArray[13].ToDouble()) ? strArray[13].ToDouble() : 0.0;
        }
        else
        {
          phase500AmpMeter.Phase1Max = strArray[0].ToInt();
          phase500AmpMeter.Phase1Min = strArray[0].ToInt();
          phase500AmpMeter.Phase1Duty = strArray[0].ToInt();
          phase500AmpMeter.Phase2Average = strArray[0].ToDouble();
          phase500AmpMeter.Phase2Max = strArray[0].ToInt();
          phase500AmpMeter.Phase2Min = strArray[0].ToInt();
          phase500AmpMeter.Phase2Duty = strArray[0].ToInt();
          phase500AmpMeter.Phase3Average = strArray[0].ToDouble();
          phase500AmpMeter.Phase3Max = strArray[0].ToInt();
          phase500AmpMeter.Phase3Min = strArray[0].ToInt();
          phase500AmpMeter.Phase3Duty = strArray[0].ToInt();
          phase500AmpMeter.TotalCurrentAccumulation = strArray[0].ToDouble();
          phase500AmpMeter.WattHours = strArray[0].ToDouble();
        }
      }
      catch
      {
  ...
  public new static Notification NotificationByApplicationID(Sensor sensor)
  {
    return new Notification()
    {
      ...
      ApplicationID = ThreePhase500AmpMeter.MonnitApplicationID,
      SnoozeDuration = 60,
    ...
    NameValueCollection viewData)
  {
    if (!string.IsNullOrEmpty(collection["Phase123Min_Manual"]) && !string.IsNullOrEmpty(collection["Phase123Max_Manual"]))
    {
      ...
      if (num1 > num2)
        num1 = num2;
      int num3 = ((double) num1 / 3.0).ToInt();
      int num4 = ((double) num2 / 3.0).ToInt();
      ThreePhase500AmpMeter.SetPhase1ThreshMin(sensor, num3);
      ThreePhase500AmpMeter.SetPhase2ThreshMin(sensor, num3);
      ThreePhase500AmpMeter.SetPhase3ThreshMin(sensor, num3);
      ThreePhase500AmpMeter.SetPhase1ThreshMax(sensor, num4);
      ThreePhase500AmpMeter.SetPhase2ThreshMax(sensor, num4);
      ThreePhase500AmpMeter.SetPhase3ThreshMax(sensor, num4);
    }
    if (!string.IsNullOrEmpty(collection["Phase123Hyst_Manual"]))
    {
      ...
      if (num5 > 600)
        num5 = 600;
      int num6 = ((double) num5 / 3.0).ToInt();
      ThreePhase500AmpMeter.SetPhase1Hyst(sensor, num6);
      ThreePhase500AmpMeter.SetPhase2Hyst(sensor, num6);
      ThreePhase500AmpMeter.SetPhase3Hyst(sensor, num6);
    }
    if (!string.IsNullOrEmpty(collection["Phase123Duty_Manual"]))
    {
      ...
      if (num7 > 600)
        num7 = 600;
      int num8 = ((double) num7 / 3.0).ToInt();
      ThreePhase500AmpMeter.SetPhase1Duty(sensor, num8);
      ThreePhase500AmpMeter.SetPhase2Duty(sensor, num8);
      ThreePhase500AmpMeter.SetPhase3Duty(sensor, num8);
    }
    if (!string.IsNullOrEmpty(collection["MeasurementInterval_Manual"]))
    {
      ...
      if (num10 > 0.0)
      {
        ...
        num9 = 1.0;
      ThreePhase500AmpMeter.setMeasurementInterval(sensor, num9);
    }
    if (!string.IsNullOrEmpty(collection["Accumulate_Manual"]))
    {
      ...
      if (num > 1 || num < 0)
        num = 1;
      ThreePhase500AmpMeter.SetAccumulate(sensor, num);
    }
    ...
    if (dataOption > 3 || dataOption < 0)
      dataOption = 1;
    ThreePhase500AmpMeter.SetDataViewOption(sensor.SensorID, dataOption);
  }
    ...
    NameValueCollection viewData)
  {
    if (!string.IsNullOrEmpty(collection["voltValue"]))
      ThreePhase500AmpMeter.SetVoltValue(sensor.SensorID, collection["voltValue"].ToDouble());
    if (!string.IsNullOrEmpty(collection["label"]))
      ThreePhase500AmpMeter.SetLabel(sensor.SensorID, collection["label"]);
    if (!string.IsNullOrEmpty(collection["PowerFactorValue"]))
      ThreePhase500AmpMeter.SetPowerFactor(sensor.SensorID, collection["PowerFactorValue"].ToDouble());
    if (!string.IsNullOrEmpty(collection["SelectedPhase"]))
      ThreePhase500AmpMeter.SetSelectedCurrentReading(sensor.SensorID, collection["SelectedPhase"].ToInt());
    if (string.IsNullOrEmpty(collection["APorEP_Manual"]))
      return;
    ThreePhase500AmpMeter.SetUseEstimatedPower(sensor.SensorID, collection["APorEP_Manual"].ToInt().ToBool());
  }
  ...
  public static bool operator ==(ThreePhase500AmpMeter left, ThreePhase500AmpMeter right)
  {
  ...
  {
    return obj is ThreePhase500AmpMeter && this.Equals((MonnitApplicationBase) (obj as ThreePhase500AmpMeter));
  }
