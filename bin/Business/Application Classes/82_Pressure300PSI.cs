...
public class Pressure300PSI : PressureNPSI, ISensorAttribute
{
  public new static long MonnitApplicationID => 82;

  public new static string ApplicationName => "Pressure 300 PSI";

  public new static eApplicationProfileType ProfileType => eApplicationProfileType.Interval;

  public override long ApplicationID => Pressure300PSI.MonnitApplicationID;
  ...
  public static Pressure300PSI Deserialize(string version, string serialized)
  {
    Pressure300PSI pressure300Psi = new Pressure300PSI();
    pressure300Psi.Pressure = serialized.ToDouble();
    return pressure300Psi;
  }

  public static Pressure300PSI Create(byte[] sdm, int startIndex)
  {
    Pressure300PSI pressure300Psi = new Pressure300PSI();
    pressure300Psi.Pressure = BitConverter.ToUInt16(sdm, startIndex).ToDouble() / 10.0;
    return pressure300Psi;
  }

  public new static void DefaultConfigurationSettings(Sensor sensor)
  {
    Pressure300PSIBase.GetDefaults(new Version(sensor.FirmwareVersion), sensor.GenerationType);
  ...
  {
    return Pressure300PSI.NotificationScaleValues();
  }
    ...
    get
    {
      ...
      else
      {
        if (!(this.Label.Value.ToLower() != "custom"))
          return (object) this.Pressure.LinearInterpolation(0.0, this.LowValue.Value.ToDouble(), 300.0, this.HighValue(300L).Value.ToDouble());
        if (this.TransformValue != null && this.TransformValue.Value != "0")
          return (object) (this.Pressure * this.TransformValue.Value.ToDouble());
      }
    ...
    NameValueCollection viewData)
  {
    ...
    double lowX = 0.0;
    double highX = 0.0;
    PressureNPSI.GetDecimalTrunkValue(sensor.SensorID).ToInt();
    if (label.ToLower() == "custom")
    {
      Pressure300PSI pressure300Psi = new Pressure300PSI();
      pressure300Psi.SetSensorAttributes(sensor.SensorID);
      lowX = pressure300Psi.LowValue.Value.ToDouble();
      highX = pressure300Psi.HighValue(300L).Value.ToDouble();
    }
    ...
    if (!string.IsNullOrEmpty(collection["Hysteresis_Manual"]) && double.TryParse(collection["Hysteresis_Manual"], out result1))
    {
      if (label.ToLower() != "custom")
      {
        double num = result1 / transform;
        sensor.Hysteresis = (num * 10.0).ToLong();
      }
      else
      {
        double o = Math.Round(result1.LinearInterpolation(lowX, 0.0, highX, savedValue));
    ...
    {
      num1 = !(label.ToLower() != "custom") ? result1.LinearInterpolation(lowX, 0.0, highX, highY) : result1 / transform;
      if (num1 < 0.0)
        num1 = 0.0;
      if (num1 > highY)
        num1 = highY;
      sensor.MinimumThreshold = (num1 * 10.0).ToLong();
    }
    ...
    {
      double num2 = !(label.ToLower() != "custom") ? result1.LinearInterpolation(lowX, 0.0, highX, highY) : result1 / transform;
      if (num2 < 0.0)
        num2 = 0.0;
      if (num2 > highY)
        num2 = highY;
      if (num1 > num2)
        num2 = num1;
      sensor.MaximumThreshold = (num2 * 10.0).ToLong();
    }
    int result2 = 0;
    if (!string.IsNullOrWhiteSpace(collection["delay"]) && int.TryParse(collection["delay"], out result2))
    {
      ...
      if (num3 > 60000)
        num3 = 60000;
      Pressure300PSI.SetStabalizationDelay(sensor, num3);
    }
    if (string.IsNullOrEmpty(collection["BasicThreshold"]) || string.IsNullOrEmpty(collection["BasicThresholdDirection"]) || !(collection["BasicThresholdDirection"] != "-1"))
      return;
    double num4 = collection["BasicThreshold"].ToDouble() / transform;
    if (collection["BasicThresholdDirection"].ToInt() == 0)
    {
      sensor.MinimumThreshold = (num4 * 10.0).ToLong();
  ...
  public static bool operator ==(Pressure300PSI left, Pressure300PSI right)
  {
  ...
  {
    return obj is Pressure300PSI && this.Equals((MonnitApplicationBase) (obj as Pressure300PSI));
  }

  public new static long DefaultMinThreshold => 0;

  public new static long DefaultMaxThreshold => 3000;
  ...
  public new static NotificationScaleInfoModel GetScalingInfo(Sensor sens)
  {
    ...
    if (scalingInfo.Label == "Custom")
    {
      ...
      scalingInfo.profileHigh = pressureNpsi.HighValue(300L).Value.ToDouble();
      scalingInfo.baseLow = 0.0;
      scalingInfo.baseHigh = 300.0;
    }
    else
