...
// Type: Monnit.LatchedDualControl
...
public class LatchedDualControl : MonnitApplicationBase, ISensorAttribute
{
  ...
  public static string ApplicationName => "Latched Dual Control";

  public static eApplicationProfileType ProfileType => eApplicationProfileType.Interval;

  public override string ChartType => "None";

  public override long ApplicationID => LatchedDualControl.MonnitApplicationID;
  ...
  {
    return (MonnitApplicationBase) LatchedDualControl.Deserialize(version, serialized);
  }
  ...
  public static LatchedDualControl Deserialize(string version, string serialized)
  {
    LatchedDualControl latchedDualControl = new LatchedDualControl();
    if (string.IsNullOrEmpty(serialized))
    {
      latchedDualControl.RelayState1 = latchedDualControl.RelayState2 = false;
    }
    else
    {
      string[] strArray = serialized.Split('|');
      latchedDualControl.RelayState1 = strArray[0].ToBool();
      latchedDualControl.RelayState2 = strArray.Length <= 1 ? strArray[0].ToBool() : strArray[1].ToBool();
    }
    return latchedDualControl;
  }

  public static LatchedDualControl Create(byte[] sdm, int startIndex)
  {
    return new LatchedDualControl()
    {
  ...
  public new static Notification NotificationByApplicationID(Sensor sensor)
  {
    return new Notification()
    {
      ...
      ApplicationID = LatchedDualControl.MonnitApplicationID,
      SnoozeDuration = 60,
    ...
    NameValueCollection viewData)
  {
    ...
    if (collection["Relay1Visibility"] != null)
    {
      ...
      {
        LatchedDualControl.ShowRelay1(sensor.SensorID);
        viewData["Relay1Visibility"] = "True";
      }
      else
      {
        LatchedDualControl.HideRelay1(sensor.SensorID);
    ...
    if (collection["Relay2Visibility"] != null)
    {
      ...
      {
        LatchedDualControl.ShowRelay2(sensor.SensorID);
        viewData["Relay2Visibility"] = "True";
      }
      else
      {
        LatchedDualControl.HideRelay2(sensor.SensorID);
    ...
    new LatchedDualControl().SetSensorAttributes(sensor.SensorID);
  }
  ...
  public new static List<byte[]> ActionControlCommand(Sensor sensor, string version)
  {
    ...
    foreach (NotificationRecorded notificationRecorded in NotificationRecorded.LoadGetNoneDevliveredMessagesForControl(sensor.SensorID))
    {
      if (notificationRecorded.NotificationType == eNotificationType.Control)
      {
        ...
        ushort time1;
        ushort time2;
        LatchedDualControl.ParseSerializedRecipientProperties(notificationRecorded.SerializedRecipientProperties, out state1, out state2, out time1, out time2);
        numArrayList.Add(LatchedDualControlBase.ControlFrame(sensor.SensorID, state1, state2, time1, time2, notificationRecorded.QueueID));
      }
    }
    ...
    try
    {
      ...
      else
      {
        ...
        foreach (NotificationRecorded notificationRecorded in NotificationRecorded.LoadNonAcknowledgedNotificationForDevice(sensor.SensorID))
        {
          if (notificationRecorded.QueueID == num1)
          {
            ...
            if (notificationRecorded.NotificationDate.AddHours(2.0) < DateTime.UtcNow && ConfigData.AppSettings("LogClearPendingExceptions", "False").ToBool())
            {
              ...
              strArray[0] = "Monnit.LatchedDualControl.ClearPendingActionControlCommand- Delivered acknowledgment was over two hours old for NotificationRecordedID: ";
        ...
        if (!flag && ConfigData.AppSettings("LogClearPendingExceptions", "False").ToBool())
          new Exception($"Monnit.LatchedDualControl.ClearPendingActionControlCommand- Delivered message not found for SensorID: {sensor.SensorID.ToString()} QueueID: {num1.ToString()}").Log();
      }
    }
    catch (Exception ex)
    {
      if (!ConfigData.AppSettings("LogClearPendingExceptions", "False").ToBool())
        return;
      ex.Log("Monnit.LatchedDualControl.ClearPendingActionControlCommand(Sensor sensor, string version, int actionCommand, bool success, byte[] data)");
    }
  }

  public override int GetHashCode() => base.GetHashCode();

  public static bool operator ==(LatchedDualControl left, LatchedDualControl right)
  {
  ...
  {
    return obj is LatchedDualControl && this.Equals((MonnitApplicationBase) (obj as LatchedDualControl));
  }

  public override bool Equals(MonnitApplicationBase right)
  {
    return right is LatchedDualControl && this.RelayState1 == (right as LatchedDualControl).RelayState1;
  }
