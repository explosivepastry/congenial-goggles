...
// Type: Monnit.ZeroToTwoHundredVolts
...
public class ZeroToTwoHundredVolts : MonnitApplicationBase, ISensorAttribute
{
  ...
  public static long MonnitApplicationID => ZeroToTwoHundredVoltsBase.MonnitApplicationID;

  public static string ApplicationName => "Voltage Meter - 200 VDC";

  public static eApplicationProfileType ProfileType => ZeroToTwoHundredVoltsBase.ProfileType;

  public override string ChartType => "Line";

  public override long ApplicationID => ZeroToTwoHundredVolts.MonnitApplicationID;
  ...
  {
    return (MonnitApplicationBase) ZeroToTwoHundredVolts.Deserialize(version, serialized);
  }
  ...
  public override string NotificationString
  {
    get
    {
      return $"{((double) this.PlotValue).ToString("#0.0##", (IFormatProvider) CultureInfo.InvariantCulture)} {this.Label.Value}";
    }
  }
  ...
  public override object PlotValue
  {
    get
    {
      return (object) this.Volts.LinearInterpolation(0.0, this.LowValue.Value.ToDouble(), 200.0, this.HighValue.Value.ToDouble());
    }
  }

  public static ZeroToTwoHundredVolts Deserialize(string version, string serialized)
  {
    return new ZeroToTwoHundredVolts()
    {
      Volts = serialized.ToDouble()
    };
  }

  public static ZeroToTwoHundredVolts Create(byte[] sdm, int startIndex)
  {
    return new ZeroToTwoHundredVolts()
    {
      Volts = BitConverter.ToUInt32(sdm, startIndex).ToDouble() / 100.0
    };
  }
  ...
  public new static Notification NotificationByApplicationID(Sensor sensor)
  {
    return new Notification()
    {
      ...
      ApplicationID = ZeroToTwoHundredVolts.MonnitApplicationID,
    ...
    NameValueCollection viewData)
  {
    ...
    if ((string.IsNullOrEmpty(collection["MaximumThreshold_Manual"]) || !double.TryParse(collection["MaximumThreshold_Manual"], out result)) && ((IEnumerable<string>) collection.AllKeys).Contains<string>("MaximumThreshold_Manual"))
      sensor.MaximumThreshold = sensor.DefaultValue<long>("DefaultMaximumThreshold");
    ZeroToTwoHundredVolts toTwoHundredVolts = new ZeroToTwoHundredVolts();
    toTwoHundredVolts.SetSensorAttributes(sensor.SensorID);
    double num1 = toTwoHundredVolts.LowValue.Value.ToDouble();
    double num2 = toTwoHundredVolts.HighValue.Value.ToDouble();
    if (!string.IsNullOrEmpty(collection["Hysteresis_Manual"]) || double.TryParse(collection["Hysteresis_Manual"], out result))
    {
      double num3 = Math.Abs((num2 - num1) / 200.0);
    ...
    if (!string.IsNullOrEmpty(collection["MinimumThreshold_Manual"]) || double.TryParse(collection["MinimumThreshold_Manual"], out result))
    {
      ...
      sensor.MinimumThreshold = x >= num1 ? (x <= num2 ? (x.LinearInterpolation(num1, 0.0, num2, 200.0) * 100.0).ToLong() : (num2.LinearInterpolation(num1, 0.0, num2, 200.0) * 100.0).ToLong()) : (num1.LinearInterpolation(num1, 0.0, num2, 200.0) * 100.0).ToLong();
    }
    if (!string.IsNullOrEmpty(collection["MaximumThreshold_Manual"]) || double.TryParse(collection["MaximumThreshold_Manual"], out result))
    {
      ...
      sensor.MaximumThreshold = x >= num1 ? (x <= num2 ? (x.LinearInterpolation(num1, 0.0, num2, 200.0) * 100.0).ToLong() : (num2.LinearInterpolation(num1, 0.0, num2, 200.0) * 100.0).ToLong()) : (num1.LinearInterpolation(num1, 0.0, num2, 200.0) * 100.0).ToLong();
    ...
    if (collection["showAdvCal"] == "on")
    {
      ...
      ZeroToTwoHundredVolts.SetShowAdvCal(sensor.SensorID, "true");
    }
    else
    {
      viewData["showAdvCal"] = "false";
      ZeroToTwoHundredVolts.SetShowAdvCal(sensor.SensorID, "false");
    }
  }
  ...
  public static void CalibrateSensor(Sensor sensor, NameValueCollection collection)
  {
    if (!(new Version(sensor.FirmwareVersion) > new Version("2.0.0.0")) && sensor.SensorTypeID != 4L)
      return;
    int num1 = (collection["observed"].ToDouble().LinearInterpolation(ZeroToTwoHundredVolts.GetLowValue(sensor.SensorID), 0.0, ZeroToTwoHundredVolts.GetHighValue(sensor.SensorID), 220.0) * 100.0).ToInt();
    int num2 = (collection["actual"].ToDouble().LinearInterpolation(ZeroToTwoHundredVolts.GetLowValue(sensor.SensorID), 0.0, ZeroToTwoHundredVolts.GetHighValue(sensor.SensorID), 220.0) * 100.0).ToInt();
    if (num1 > 0 && num2 > 0)
    {
      sensor.Calibration1 = (long) ((long) num2 * sensor.Calibration1 / (long) num1).ToInt();
      sensor.Save();
    }
  }
    ...
    NameValueCollection viewData)
  {
    if (!string.IsNullOrEmpty(collection["lowValue"]))
      ZeroToTwoHundredVolts.SetLowValue(sensor.SensorID, collection["lowValue"].ToDouble());
    if (!string.IsNullOrEmpty(collection["highValue"]))
      ZeroToTwoHundredVolts.SetHighValue(sensor.SensorID, collection["highValue"].ToDouble());
    if (string.IsNullOrEmpty(collection["label"]))
      return;
    ZeroToTwoHundredVolts.SetLabel(sensor.SensorID, collection["label"]);
  }
  ...
  public static string HystForUI(Sensor sensor)
  {
    ...
    toTwoHundredVolts.SetSensorAttributes(sensor.SensorID);
    double num1 = toTwoHundredVolts.LowValue.Value.ToDouble();
    double num2 = Math.Abs((toTwoHundredVolts.HighValue.Value.ToDouble() - num1) / 200.0);
    return sensor.Hysteresis == (long) uint.MaxValue ? "" : ((double) sensor.Hysteresis / 100.0 * num2).ToString();
  }
  ...
  public static string MinThreshForUI(Sensor sensor)
  {
    ZeroToTwoHundredVolts toTwoHundredVolts = new ZeroToTwoHundredVolts();
    toTwoHundredVolts.SetSensorAttributes(sensor.SensorID);
    double lowY = toTwoHundredVolts.LowValue.Value.ToDouble();
    double highY = toTwoHundredVolts.HighValue.Value.ToDouble();
    return sensor.MinimumThreshold == (long) uint.MaxValue ? "" : (sensor.MinimumThreshold.ToDouble() / 100.0).LinearInterpolation(0.0, lowY, 200.0, highY).ToString();
  }
  ...
  public static string MaxThreshForUI(Sensor sensor)
  {
    ...
    double lowY = toTwoHundredVolts.LowValue.Value.ToDouble();
    double highY = toTwoHundredVolts.HighValue.Value.ToDouble();
    return sensor.MaximumThreshold == (long) uint.MaxValue ? "" : ((double) sensor.MaximumThreshold / 100.0).LinearInterpolation(0.0, lowY, 200.0, highY).ToString();
  }

  public override int GetHashCode() => base.GetHashCode();

  public static bool operator ==(ZeroToTwoHundredVolts left, ZeroToTwoHundredVolts right)
  {
  ...
  {
    return obj is ZeroToTwoHundredVolts && this.Equals((MonnitApplicationBase) (obj as ZeroToTwoHundredVolts));
  }

  public override bool Equals(MonnitApplicationBase right)
  {
    return right is ZeroToTwoHundredVolts && this.Volts == (right as ZeroToTwoHundredVolts).Volts;
  }
  ...
  public new static NotificationScaleInfoModel GetScalingInfo(Sensor sens)
  {
    ...
    ZeroToTwoHundredVolts toTwoHundredVolts = new ZeroToTwoHundredVolts();
    toTwoHundredVolts.SetSensorAttributes(sens.SensorID);
    scalingInfo.Label = toTwoHundredVolts.Label.Value.ToStringSafe();
    scalingInfo.profileLow = toTwoHundredVolts.LowValue.Value.ToDouble();
    scalingInfo.profileHigh = toTwoHundredVolts.HighValue.Value.ToDouble();
    scalingInfo.baseLow = 0.0;
    scalingInfo.baseHigh = 200.0;
    return scalingInfo;
  }
}
