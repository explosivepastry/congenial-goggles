...
public class Pressure750PSI : PressureNPSI, ISensorAttribute
{
  public new static long MonnitApplicationID => 144 /*0x90*/;

  public new static string ApplicationName => "Pressure 750 PSI";

  public new static eApplicationProfileType ProfileType => eApplicationProfileType.Interval;

  public override long ApplicationID => Pressure750PSI.MonnitApplicationID;
  ...
  public static Pressure750PSI Deserialize(string version, string serialized)
  {
    Pressure750PSI pressure750Psi = new Pressure750PSI();
    pressure750Psi.Pressure = serialized.ToDouble();
    return pressure750Psi;
  }

  public static Pressure750PSI Create(byte[] sdm, int startIndex)
  {
    Pressure750PSI pressure750Psi = new Pressure750PSI();
    pressure750Psi.Pressure = BitConverter.ToUInt16(sdm, startIndex).ToDouble() / 10.0;
    return pressure750Psi;
  }
  ...
  {
    return Pressure750PSI.NotificationScaleValues();
  }
    ...
    get
    {
      ...
      else
      {
        if (!(this.Label.Value.ToLower() != "custom"))
          return (object) this.Pressure.LinearInterpolation(0.0, this.LowValue.Value.ToDouble(), 750.0, this.HighValue(750L).Value.ToDouble());
        if (this.TransformValue != null && this.TransformValue.Value != "0")
          return (object) (this.Pressure * this.TransformValue.Value.ToDouble());
      }
    ...
    NameValueCollection viewData)
  {
    ...
    {
      Pressure750PSI pressure750Psi = new Pressure750PSI();
      pressure750Psi.SetSensorAttributes(sensor.SensorID);
      lowX = pressure750Psi.LowValue.Value.ToDouble();
      highX = pressure750Psi.HighValue(750L).Value.ToDouble();
    }
    ...
    if (!string.IsNullOrEmpty(collection["Hysteresis_Manual"]) && double.TryParse(collection["Hysteresis_Manual"], out result1))
    {
      ...
      {
        double num3 = Math.Abs((highX - lowX) / 750.0);
    ...
    if (!string.IsNullOrWhiteSpace(collection["delay"]) && int.TryParse(collection["delay"], out result2))
    {
      ...
      if (num7 > 60000)
        num7 = 60000;
      Pressure750PSI.SetStabalizationDelay(sensor, num7);
    }
  ...
  public static bool operator ==(Pressure750PSI left, Pressure750PSI right)
  {
  ...
  {
    return obj is Pressure750PSI && this.Equals((MonnitApplicationBase) (obj as Pressure750PSI));
  }

  public new static long DefaultMinThreshold => 0;

  public new static long DefaultMaxThreshold => 750;
  ...
  {
    Pressure750PSIBase.GetDefaults(new Version(sensor.FirmwareVersion), sensor.GenerationType);
  ...
  public new static NotificationScaleInfoModel GetScalingInfo(Sensor sens)
  {
    ...
    if (scalingInfo.Label == "Custom")
    {
      ...
      scalingInfo.profileHigh = pressureNpsi.HighValue(750L).Value.ToDouble();
      scalingInfo.baseLow = 0.0;
      scalingInfo.baseHigh = 750.0;
    }
    else
