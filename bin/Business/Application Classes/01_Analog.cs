...
public class Analog : MonnitApplicationBase, ISensorAttribute
{
  ...
  public static long MonnitApplicationID => AnalogBase.MonnitApplicationID;

  public static string ApplicationName => "1.2 VDC";

  public static eApplicationProfileType ProfileType => AnalogBase.ProfileType;

  public override string ChartType => "Line";

  public override long ApplicationID => Analog.MonnitApplicationID;
  ...
  public override List<AppDatum> Datums
  {
    get
    {
      ...
      {
        new AppDatum(eDatumType.Voltage0To1Point2, "Volts", this.Volts)
      });
    }
  }
  ...
  public override List<string> GetPlotLabels(long sensorID)
  {
    ...
    {
      Analog.GetLabel(sensorID)
    });
  }

  public override MonnitApplicationBase _Deserialize(string version, string serialized)
  {
    return (MonnitApplicationBase) Analog.Deserialize(version, serialized);
  }
  ...
  public override object PlotValue
  {
    get
    {
      return (object) this.Volts.LinearInterpolation(0.0, this.LowValue.Value.ToDouble(), 1.2, this.HighValue.Value.ToDouble());
    }
  }

  public static Analog Deserialize(string version, string serialized)
  {
    return new Analog() { Volts = serialized.ToDouble() };
  }

  public static Analog Create(byte[] sdm, int startIndex)
  {
    return new Analog()
    {
      Volts = (double) BitConverter.ToInt16(sdm, startIndex) / 1000.0
    };
  }
  ...
  public new static Notification NotificationByApplicationID(Sensor sensor)
  {
    return new Notification()
    {
      ...
      ApplicationID = Analog.MonnitApplicationID,
    ...
    NameValueCollection viewData)
  {
    if (!string.IsNullOrEmpty(collection["lowValue"]))
      Analog.SetLowValue(sensor.SensorID, collection["lowValue"].ToDouble());
    if (!string.IsNullOrEmpty(collection["highValue"]))
      Analog.SetHighValue(sensor.SensorID, collection["highValue"].ToDouble());
    if (string.IsNullOrEmpty(collection["label"]))
      return;
    Analog.SetLabel(sensor.SensorID, collection["label"]);
  }
    ...
    NameValueCollection viewData)
  {
    ...
    analog.SetSensorAttributes(sensor.SensorID);
    double num1 = analog.LowValue.Value.ToDouble();
    double num2 = analog.HighValue.Value.ToDouble();
    if (!string.IsNullOrEmpty(collection["Hysteresis_Manual"]))
    {
      if (double.TryParse(collection["Hysteresis_Manual"], out result))
      {
        double num3 = Math.Abs((num2 - num1) / 1.2);
    ...
    if (!string.IsNullOrEmpty(collection["MinimumThreshold_Manual"]))
    {
      if (double.TryParse(collection["MinimumThreshold_Manual"], out result))
      {
        ...
        sensor.MinimumThreshold = x >= num1 ? (x <= num2 ? (x.LinearInterpolation(num1, 0.0, num2, 1.2) * 1000.0).ToLong() : (num2.LinearInterpolation(num1, 0.0, num2, 1.2) * 1000.0).ToLong()) : (num1.LinearInterpolation(num1, 0.0, num2, 1.2) * 1000.0).ToLong();
      }
      else
        sensor.MinimumThreshold = (num1.LinearInterpolation(num1, 0.0, num2, 1.2) * 1000.0).ToLong();
    }
    if (!string.IsNullOrEmpty(collection["MaximumThreshold_Manual"]))
    {
      if (double.TryParse(collection["MaximumThreshold_Manual"], out result))
      {
        ...
        sensor.MaximumThreshold = x >= num1 ? (x <= num2 ? (x.LinearInterpolation(num1, 0.0, num2, 1.2) * 1000.0).ToLong() : (num2.LinearInterpolation(num1, 0.0, num2, 1.2) * 1000.0).ToLong()) : (num1.LinearInterpolation(num1, 0.0, num2, 1.2) * 1000.0).ToLong();
        if (sensor.MinimumThreshold > sensor.MaximumThreshold)
          sensor.MaximumThreshold = sensor.MinimumThreshold;
      }
      else
        sensor.MaximumThreshold = (num2.LinearInterpolation(num1, 0.0, num2, 1.2) * 1000.0).ToLong();
    }
  ...
  public static void CalibrateSensor(Sensor sensor, NameValueCollection collection)
  {
    if (sensor.SensorTypeID != 4L)
    {
      ...
      {
        int num1 = (collection["observed"].ToDouble().LinearInterpolation(Analog.GetLowValue(sensor.SensorID), 0.0, Analog.GetHighValue(sensor.SensorID), 1.2) * 1000.0).ToInt();
        int num2 = (collection["actual"].ToDouble().LinearInterpolation(Analog.GetLowValue(sensor.SensorID), 0.0, Analog.GetHighValue(sensor.SensorID), 1.2) * 1000.0).ToInt();
        if (num1 > 0 && num2 > 0)
        {
    ...
    {
      int num4 = (collection["observed"].ToDouble().LinearInterpolation(Analog.GetLowValue(sensor.SensorID), 0.0, Analog.GetHighValue(sensor.SensorID), 1.2) * 1000.0).ToInt();
      int num5 = (collection["actual"].ToDouble().LinearInterpolation(Analog.GetLowValue(sensor.SensorID), 0.0, Analog.GetHighValue(sensor.SensorID), 1.2) * 1000.0).ToInt();
      if (num4 > 0 && num5 > 0)
      {
        int num6 = ((long) (num4 * 1000000) / sensor.Calibration1).ToInt();
        sensor.Calibration1 = (long) (num5 * 1000000 / num6);
      }
    }
    sensor.Save();
  }
  ...
  public static string HystForUI(Sensor sensor)
  {
    ...
    analog.SetSensorAttributes(sensor.SensorID);
    double num1 = analog.LowValue.Value.ToDouble();
    double num2 = Math.Abs((analog.HighValue.Value.ToDouble() - num1) / 1.2);
    return ((double) sensor.Hysteresis / 1000.0 * num2).ToString();
  }
  ...
  public static string MinThreshForUI(Sensor sensor)
  {
    Analog analog = new Analog();
    analog.SetSensorAttributes(sensor.SensorID);
    double lowY = analog.LowValue.Value.ToDouble();
    double highY = analog.HighValue.Value.ToDouble();
    return sensor.MinimumThreshold == (long) uint.MaxValue ? "" : (sensor.MinimumThreshold.ToDouble() / 1000.0).LinearInterpolation(0.0, lowY, 1.2, highY).ToString();
  }
  ...
  public static string MaxThreshForUI(Sensor sensor)
  {
    ...
    double lowY = analog.LowValue.Value.ToDouble();
    double highY = analog.HighValue.Value.ToDouble();
    return sensor.MaximumThreshold == (long) uint.MaxValue ? "" : ((double) sensor.MaximumThreshold / 1000.0).LinearInterpolation(0.0, lowY, 1.2, highY).ToString();
  }

  public override int GetHashCode() => base.GetHashCode();

  public static bool operator ==(Analog left, Analog right)
  {
  ...
  {
    return obj is Analog && this.Equals((MonnitApplicationBase) (obj as Analog));
  }

  public override bool Equals(MonnitApplicationBase right)
  {
    return right is Analog && this.Volts == (right as Analog).Volts;
  }
  ...
  public static long DefaultMaxThreshold => 1200;
  ...
  public new static NotificationScaleInfoModel GetScalingInfo(Sensor sens)
  {
    ...
    Analog analog = new Analog();
    analog.SetSensorAttributes(sens.SensorID);
    scalingInfo.Label = analog.Label.Value.ToStringSafe();
    scalingInfo.profileLow = analog.LowValue.Value.ToDouble();
    scalingInfo.profileHigh = analog.HighValue.Value.ToDouble();
    scalingInfo.baseLow = 0.0;
    scalingInfo.baseHigh = 1.2;
    return scalingInfo;
  }
}
