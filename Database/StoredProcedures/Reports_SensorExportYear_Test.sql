...
/****** Object:  StoredProcedure [dbo].[Report_SensorExportYear_Test]    Script Date: 7/1/2024 9:03:14 AM ******/
SET ANSI_NULLS ON
GO
...
CREATE PROCEDURE [dbo].[Report_SensorExportYear_Test]
...
--DECLARE @FromDate DATETIME = dateadd(year, -1, getutcdate())
--DECLARE @ToDate DATETIME = getutcdate()
--DECLARE @Default_OwnerAccountID BIGINT = 16737

if @FromDate < dateadd(year, -1, getutcdate())
...
DECLARE @Limit INT = 60000


SET @AccountID = (Select AccountID from Sensor where SensorID = @SensorID)
IF (SELECT ISNULL(StartDate, DATEADD(YEAR, -1, GETUTCDATE())) FROM Sensor WHERE SensorID = @SensorID) > @FromDate
SET @FromDate = (SELECT ISNULL(StartDate, DATEADD(YEAR, -1, GETUTCDATE())) FROM Sensor WHERE SensorID = @SensorID)
...
WHILE @StartMonth <= @EndMonth AND @RowCount <= @Limit AND @AccountID = @Default_OwnerAccountID
BEGIN
    ...
    select 
     ...
     from '+ @Year +'.dbo.DataMessage_'+CONVERT(VARCHAR(6), @StartMonth, 112)+' d with (NOLOCK) 
     INNER JOIN Sensor s WITH (NOLOCK) ON s.SensorID = d.SensorID
    WHERE d.SensorID = '+CONVERT(VARCHAR(20), @SensorID)+'
      AND MessageDate BETWEEN '''+CONVERT(VARCHAR(30), @FromDate)+''' and '''+CONVERT(VARCHAR(30),@ToDate)+''''
...
IF @RowCount > 60000
set @ToDate = (select max(MessageDate) from #DMtemp)
