...
/****** Object:  StoredProcedure [dbo].[Report_NetworkExportInternal]    Script Date: 7/1/2024 8:47:00 AM ******/
SET ANSI_NULLS ON
GO
...
CREATE PROCEDURE [dbo].[Report_NetworkExportInternal]
...
SELECT 
 ...
 SELECT
 SensorID, ApplicationID
 INTO #SensorList
 FROM Sensor where CSNetID = @NetworkID
...
BEGIN
    ...
    select 
      ...
      ApplicationID = ISNULL(s.ApplicationID, 0)
     from '+ @Year +'.dbo.DataMessage_'+CONVERT(VARCHAR(6), @StartMonth, 112)+' d with (NOLOCK) 
    INNER JOIN #SensorList s on d.sensorID = s.SensorID
    WHERE MessageDate BETWEEN '''+CONVERT(VARCHAR(30), @FromDate)+''' and '''+CONVERT(VARCHAR(30),@ToDate)+''''
