...
/****** Object:  StoredProcedure [dbo].[Report_LogNotesReportNKarta]    Script Date: 6/28/2024 4:15:17 PM ******/
SET ANSI_NULLS ON
GO
...
CREATE PROCEDURE [dbo].[Report_LogNotesReportNKarta]
...
'
  SELECT 
  ReadingTimeStamp = DATEADD(HOUR, '+CONVERT(VARCHAR(3), @HourOffset)+', dm.[MessageDate]), 
  ReadingValue = dm.Data,
  dm.[SensorID], 
  [USER]          = c.[FirstName]+'' ''+c.[LastName], 
  NoteDate = DATEADD(HOUR, '+CONVERT(VARCHAR(3), @HourOffset)+', dmn.[NoteDate]), 
  dmn.[Note]
FROM DataMessage dm
INNER JOIN Sensor s on dm.SensorID = s.SensorID
INNER JOIN dbo.[DataMessageNote] dmn WITH (NOLOCK) ON dmn.DataMessageGUID = dm.DataMessageGUID and dmn.MessageDate = dm.MessageDate
INNER JOIN dbo.[Customer] c WITH (NOLOCK) ON c.CustomerID = dmn.CustomerID
WHERE dm.MessageDate BETWEEN '''+CONVERT(VARCHAR(30), @FromDate)+ ''' AND ''' + CONVERT(VARCHAR(200), @ToDate) + '''
  AND s.AccountID = ' + CONVERT(VARCHAR(20), @Default_OwnerAccountID)+'
ORDER BY dm.MessageDate, SensorID';
