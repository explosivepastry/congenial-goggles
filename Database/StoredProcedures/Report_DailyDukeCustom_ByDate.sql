...
/****** Object:  StoredProcedure [dbo].[Report_DailyDukeCustom_ByDate]    Script Date: 6/24/2024 2:53:31 PM ******/
SET ANSI_NULLS ON
GO
...
CREATE PROCEDURE [dbo].[Report_DailyDukeCustom_ByDate]
	@Default_OwnerAccountID BIGINT = 1,
  @FromDate DATETIME
  AS
...
WHERE a.AccountID = 21974

SET @FromDate =  CONVERT(DATETIME, CONVERT(VARCHAR(11),@FromDate, 120) + '04:00')
...
[Tree]                    = SUBSTRING((SELECT 
                          ...
                          FROM dbo.[Split](RIGHT(a.[AccountIDTree], LEN(a.[AccountIDTree]) - (CHARINDEX(CONVERT(VARCHAR(300), 21974), a.[AccountIDTree])) + 1),'*') s
...
--DROP TABLE IF EXISTS #Table7
--DROP TABLE IF EXISTS #Table8
--drop table #InactiveGateways
--drop table #InactiveSensors
--drop table #TempGM
