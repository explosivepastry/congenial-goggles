...
<%
    ...
    if (Model.ScaleID != long.MinValue)
    {
        ...
        label = Monnit.Application_Classes.DataTypeClasses.AmpHours.GetLabel(Model.ScaleID);
        lowVal = Monnit.Application_Classes.DataTypeClasses.AmpHours.GetLowValue(Model.ScaleID);
        highVal = Monnit.Application_Classes.DataTypeClasses.AmpHours.GetHighValue(Model.ScaleID);
    }
    sensorIDList = Sensor.LoadByAccountID(MonnitSession.CurrentCustomer.AccountID).Where(appid => { return appid.GetDatumTypes().Contains(eDatumType.AmpHours); }).Select(sensid => sensid.SensorID).ToList();
    ...
    <td>
        <select id="scaleid" name="scaleid">
            <% for (int i = 0; i < sensorIDList.Count; i++)
               {
                   tempLabel = Monnit.Application_Classes.DataTypeClasses.AmpHours.GetLabel(sensorIDList[i]);%>
...
<tr>
    ...
    <td>
       <span class="scaleLabel" > <%: Html.Label(label) %></span> <span class="baseLabel">(<%: (Model.CompareValue.ToDouble()).ToString()%> Ah</span>
    </td>
</tr>
...
<input type="hidden" id="NotificationDatum" name="NotificationDatum" value="<%:(int)eDatumType.AmpHours %>" />
