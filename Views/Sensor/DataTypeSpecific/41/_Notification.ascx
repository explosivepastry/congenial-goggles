...
<%
    ...
    if (Model.ScaleID > 0)
    {
        ...
        label = ZeroToFiveVolts.GetLabel(Model.ScaleID);
        lowVal = ZeroToFiveVolts.GetLowValue(Model.ScaleID);
        highVal = ZeroToFiveVolts.GetHighValue(Model.ScaleID);
    ...
    double revertedVal = Model.CompareValue.ToDouble().LinearReversion(lowVal, 0, highVal, 5);
...
<tr>
    ...
    <td>
        ...
        <input type="hidden" id="CompareValue" name="CompareValue" value="<%:Model.CompareValue %>" />
    </td>
    <td>
...
<input id="NotificationDatum" value="<%: (int)eDatumType.Voltage0To5 %>" type="hidden" />
