...
<%
    ...
    if (Model.ScaleID > 0)
    {
        ...
        label = VoltageMeter500VAC.GetLabel(Model.ScaleID);
        lowVal = VoltageMeter500VAC.GetLowValue(Model.ScaleID);
        highVal = VoltageMeter500VAC.GetHighValue(Model.ScaleID);
    ...
    else
    {
        lowVal = 0;
        highVal = 500;
        sensorIDList = Sensor.LoadByAccountID(MonnitSession.CurrentCustomer.AccountID).Where(appid => { return appid.ApplicationID == 122 || appid.ApplicationID == 32; }).Select(sensid => sensid.SensorID).ToList();
    }

    double revertedVal = Model.CompareValue.ToDouble().LinearInterpolation(0, lowVal, 500, highVal, 2);
    ...
    <td>
        <select id="ScaleID" name="ScaleID">
            <% for (int i = 0; i < sensorIDList.Count; i++)
               {
                   tempLabel = AC_DC_500V.GetLabel(sensorIDList[i]);%>
...
<input id="NotificationDatum" value="<%: (int)eDatumType.Voltage0To500 %>" type="hidden"/>
    ...
    $(function () {
        ...
        $('#ScaleID').blur(function () {
            ...
            if ($('#ScaleID').val() > 0) {
                ...
                var scaleID = $('#ScaleID').val();
                var comparVal = $("#CompareValue").val();
                var notificationDatum = $('#NotificationDatum').val();
