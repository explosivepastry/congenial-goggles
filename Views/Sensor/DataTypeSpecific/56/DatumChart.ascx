...
<div class="row">
    <div class="col-12 col-lg-3 col-xl-3" style="text-align: center;">
        <%=Model.GroupSensor.Sensor.SensorName %><br />
        <%=HttpUtility.UrlDecode(Model.GroupSensor.Alias) %><br />
        <a target="_blank" href="/Overview/SensorChart/<%=Model.GroupSensor.SensorID %>" role="link" class="btn btn-grey  hidden-xs hidden-sm hidden-md">
            <%=Html.GetThemedSVG("chart") %>
            &nbsp;<%: Html.TranslateTag("Details","Details")%></a> <a href="/Overview/SensorHome/<%=Model.GroupSensor.SensorID %>" role="link" target="_blank" class="btn btn-grey  hidden-xs hidden-sm hidden-md"><%=Html.GetThemedSVG("list") %>&nbsp;<%: Html.TranslateTag("History","History")%></a>
    </div>
