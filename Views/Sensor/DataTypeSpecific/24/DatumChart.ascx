...
<%    //DoorData = 24
    ...
    bool Reverse = OpenClosed.GetMagnetPresentValue(Model.GroupSensor.Sensor.SensorID) == "Introduced";
            ...
            baseOption: {
                tooltip: {
                    ...
                    formatter: function (params) {
                        ...
                        return "Date: " + echarts.format.formatTime('<%=(MonnitSession.CurrentCustomer.Preferences["Date Format"] + ' ' + MonnitSession.CurrentCustomer.Preferences["Time Format"]).Replace(" tt","") %>', date, true) + '<br/>' +
                           'Value: ' + (params.value[1] == 1 ? '<%=OneValue%>' : '<%=ZeroValue%>');
                    }
                },
                grid: {
                    // y2: 80,
                ...
                xAxis: [
                    {
                        ...
                        axisLabel: {
                            formatter: function (params) {
                                ...
                                return echarts.format.formatTime('<%=(MonnitSession.CurrentCustomer.Preferences["Time Format"]).Replace(" tt","") %>', date, true) + ' '  + echarts.format.formatTime('MM/dd', date, true);
                            }
