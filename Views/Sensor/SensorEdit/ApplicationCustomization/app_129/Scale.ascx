...
<form action="/Overview/SensorScale/<%:Model.SensorID %>" id="SensorScale_<%:Model.SensorID %>" method="post">
    ...
    <%
        string volts = ThreePhase500AmpMeter.GetVoltValue(Model.SensorID).ToString();
        string Hours = ThreePhase500AmpMeter.GetLabel(Model.SensorID);
        double estimatedPowerFactor = ThreePhase500AmpMeter.GetPowerFactor(Model.SensorID);
        int phaseSelection = ThreePhase500AmpMeter.GetSelectedCurrentReading(Model.SensorID);
        int useEstimatedPower = ThreePhase500AmpMeter.GetUseEstimatedPower(Model.SensorID).ToBool().ToInt();
    %>
   ...
   <div class="row sensorEditForm">
        ...
        <div class="col sensorEditFormInput">
            <select id="label" name="label" class="form-select">
                <option value="Ah" <%:  Hours == "Ah" ?"selected":""%> <%: Hours == "Ah"?"selected":"" %>>Amp Hours</option>
                <option value="MJ" <%:  Hours == "MJ" ?"selected":""%> <%: Hours == "MJ"?"selected":"" %>>MegaJoule</option>
                <option value="kWh" <%:  Hours == "kWh" ?"selected":""%> <%: Hours == "kWh"?"selected":"" %>>Kilowatt Hours</option>
            </select>
        </div>
    </div>
    ...
    <div class="row sensorEditForm apDiv voltRow2" style="display: none;">
        ...
        <div class="col sensorEditFormInput">
            <input type="number" class="form-control" name="PowerFactorValue" id="PowerFactorValue" min="0" value="<%=estimatedPowerFactor %>" />
        </div>
    </div>
