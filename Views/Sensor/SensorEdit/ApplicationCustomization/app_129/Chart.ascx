...
<%
    ...
    string label = ThreePhase500AmpMeter.GetLabel(Model.Sensors[0].SensorID);


    foreach (DataMessage item in list)
    {
        ThreePhase500AmpMeter Data = Monnit.ThreePhase500AmpMeter.Deserialize(Model.Sensors[0].FirmwareVersion, item.Data);
