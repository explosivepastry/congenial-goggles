...
<%
    ...
    double maxAware = (Model.Sensors[0].MaximumThreshold / 1000.0);
    double minAware = (Model.Sensors[0].MinimumThreshold / 1000.0);


    StringBuilder dataVals = new StringBuilder();
    string OneValue = "Flowing";
    string ZeroValue = "Not Flowing";

    foreach (DataMessage item in list)
    {
        dataVals.Append("[new Date(" + Monnit.TimeZone.GetLocalTimeById(item.MessageDate, MonnitSession.CurrentCustomer.Account.TimeZoneID).ToEpochTime() + ")," + ((item.AppBase.PlotValue.ToInt() > 0) ? 1 : 0) + "," + ((item.State & 2) == 2).ToInt() + ",'" + item.DataMessageGUID + "'],");
    }
                ...
                tooltip: {
                    ...
                    formatter: function (params) {
                        ...
                        return "Date: " + DatePreferenceFormat(params.value[0], '<%=(MonnitSession.CurrentCustomer.Preferences["Date Format"])%>') + ' ' + TimePreferenceFormat(params.value[0], '<%=(MonnitSession.CurrentCustomer.Preferences["Time Format"]).Replace(" tt","") %>') + '<br/>' +
                            'Value: ' + (params.value[1] == 1 ? '<%=ZeroValue%>' : '<%=OneValue%>') + '<br/>' +
