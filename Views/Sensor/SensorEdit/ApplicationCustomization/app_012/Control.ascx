...
<% 
    ...
    Control_1 ControlMessage = null;
    if (Model.LastDataMessage != null)
    {
        ControlMessage = (Control_1)MonnitApplicationBase.LoadMonnitApplication(Model.FirmwareVersion, Model.LastDataMessage.Data, Model.ApplicationID, Model.SensorID);
    }
%>
<style>
    .green {
                            ...
                            if (s != "0")
                            {
                                ...
                                if (!RelayWritten)
                                {
                                    if (isRelayOne)
                                    {
                                        RelayWritten = true;
            %>
                    <td><%:Control_1.Relay1Name(Model.SensorID)%></td>
                    <%}
                        else
                        {
                            RelayWritten = true; %>
                    <td><%:Control_1.Relay2Name(Model.SensorID)%></td>
                    <%}
                        }
                        status++;
                        if (status == 1)
