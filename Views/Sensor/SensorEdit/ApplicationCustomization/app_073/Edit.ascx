...
<% Account acc = Account.Load(Model.AccountID);
    if (acc.CurrentSubscription.AccountSubscriptionType.Can("sensor_advanced_edit") && MonnitSession.CustomerCan("Sensor_Advanced_Configuration"))
    {
        ...
        Html.RenderPartial("~/Views/Sensor/SensorEdit/ApplicationCustomization/app_073/_MaxThreshold.ascx", Model);
        Html.RenderPartial("~/Views/Sensor/SensorEdit/ApplicationCustomization/app_073/_MinThreshold.ascx", Model);
        Html.RenderPartial("~/Views/Sensor/SensorEdit/ApplicationCustomization/app_073/_Accumulation.ascx", Model);
