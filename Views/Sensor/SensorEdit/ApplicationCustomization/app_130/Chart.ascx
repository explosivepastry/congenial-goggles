...
<%
    ...
    StringBuilder RiseFallTimeDataVals = new StringBuilder();



    foreach (DataMessage item in list)
    {
        TriggeredTilt Data = (TriggeredTilt)MonnitApplicationBase.LoadMonnitApplication(Model.Sensors[0].FirmwareVersion, item.Data, Model.Sensors[0].ApplicationID, Model.Sensors[0].SensorID);
        // TriggeredTilt Data = Monnit.TriggeredTilt.Deserialize(Model.Sensors[0].FirmwareVersion, item.Data);
        ...
        RiseFallTimeDataVals.Append("[new Date(" + Monnit.TimeZone.GetLocalTimeById(item.MessageDate, MonnitSession.CurrentCustomer.Account.TimeZoneID).ToEpochTime() + ")," + Data.RiseFallTime.ToString() + "," + ((item.State & 2) == 2).ToInt() + ", 'RiseFallTime' ,'" + item.DataMessageGUID +  "' ],");
    }
%>
                ...
                tooltip: {
                    ...
                    formatter: function (params) {
                        ...
                        if (params.value[3] == 'Pitch') {
                            ...
                            return "Date: " + DatePreferenceFormat(params.value[0], '<%=(MonnitSession.CurrentCustomer.Preferences["Date Format"])%>') + ' ' + TimePreferenceFormat(params.value[0], '<%=(MonnitSession.CurrentCustomer.Preferences["Time Format"]).Replace(" tt","") %>') + '<br/>' +
                                'Value: ' + params.value[1] + '  Rise/Fall Time' + '<br/>' +
                                'Status: ' + (params.value[2] == 1 ? 'Aware' : 'Not Aware');
                        }
                    }
                },
                ...
                series: [
                    {
                        ...
                        type: 'line',
                        showAllSymbol: true,
                        data: [<%=RiseFallTimeDataVals.ToString().TrimEnd(',')%>],
