...
<%
    ...
    StringBuilder RollDataVals = new StringBuilder();



    foreach (DataMessage item in list)
    {
        Tilt Data = Monnit.Tilt.Deserialize(Model.Sensors[0].FirmwareVersion, item.Data);
        PitchDataVals.Append("[new Date(" + Monnit.TimeZone.GetLocalTimeById(item.MessageDate, MonnitSession.CurrentCustomer.Account.TimeZoneID).ToEpochTime() + ")," + Data.PitchWithOffset.ToDouble().ToString("#0.#") + "," + ((item.State & 2) == 2 ).ToInt() + ", 'Pitch' ,'" + item.DataMessageGUID +  "' ],");
        RollDataVals.Append("[new Date(" + Monnit.TimeZone.GetLocalTimeById(item.MessageDate, MonnitSession.CurrentCustomer.Account.TimeZoneID).ToEpochTime() + ")," + Data.RollWithOffSet.ToString("#0.##") + "," + ((item.State & 2) == 2 ).ToInt() + ", 'Roll' ,'" + item.DataMessageGUID +  "' ],");
    }   
%>
                ...
                series: [
                   {
                       ...
                       type: 'line',
                       showAllSymbol: true,
                       data: [<%=RollDataVals.ToString().TrimEnd(',')%>],
