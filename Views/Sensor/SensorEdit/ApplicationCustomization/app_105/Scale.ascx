...
<form action="/Overview/SensorScale/<%:Model.SensorID %>" id="SensorScale_<%:Model.SensorID %>" method="post">
    ...
    <br />
      ...
      <div class="row sensorEditForm" id="hideMe" style="display: none;">
        <div class="col-12 col-md-3">
             <%: Html.TranslateTag("Sensor/ApplicationCustomization/app_067|Tank Depth:","Tank Depth:")%>
        </div>
        <div class="col sensorEditFormInput">
    ...
    <script>
        ...
        $(document).ready(function () {
            ...
            let showDepthOriginal = '<%: showDepth.ToString().ToLower() %>'

            $('#ShowDepth').change(function (e) {
                e.preventDefault();

                if ($('#ShowDepth').val() != showDepthOriginal) {
                    let ocVals = {};
                    ocVals.text = '<%: Html.TranslateTag("Changing this value will remove this sensor from all notifications.") %>';
                    ocVals.url = "/Sensor/RemoveFromAllNotifications/";
                    ocVals.params = { id: "<%: Model.SensorID %>" };
                    ocVals.callback = showDepthChange;
                    ocVals.cancelCallback = function () { $('#ShowDepth').val(showDepthOriginal); };
                    openConfirm(ocVals);
                } else {
                    showDepthChange();
                }
            ...
            function showDepthChange(data) {

                if (data != null && data != 'Success') {
                    $('#ShowDepth').val(showDepthOriginal);
                    showAlertModal(data);
                }

                if ($('#ShowDepth').val() == 'true') {
                    if (!isANumber($('#TankDepth').val())) {
                        $('#TankDepth').val('<%: tankDepth %>');
                    }
                    $("#hideMe").show();
                } else {
    ...
    <script type="text/javascript">
        ...
        $(document).ready(function () {
            ...
            $('#save').click(function () {
                if ($('#ShowDepth').val() == 'true' && (!isANumber($('#TankDepth').val()) || $('#TankDepth').val() < 0)) {
                    showAlertModal("Tank Depth must be a non-negative number.");
                } else {
