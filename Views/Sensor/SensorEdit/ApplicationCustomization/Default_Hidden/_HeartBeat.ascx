...
<div class="form-group">
    <div class="bold col-md-3 col-sm-3 col-xs-12">
        <%if (Model.ApplicationID != 23 || Model.ApplicationID == 23 && new Version(Model.FirmwareVersion) > new Version("1.2.0.3"))
            { %>
        <%: Html.TranslateTag("Sensor/SensorEdit/ApplicationCustomization/Default_Hidden/_HeartBeat|Gateway Check-in Frequency","Gateway Check-in Frequency")%> (<%:Html.TranslateTag("Minutes") %>)
        <%}
            ...
            { %>
    <%: Html.TranslateTag("Sensor/SensorEdit/ApplicationCustomization/Default_Hidden/_HeartBeat|Time Before Motion Rearm","Time Before Motion Rearm")%>   
    <%} %>
    </div>
    <div class="col-md-9 col-sm-9 col-xs-12 mdBox">

        <input class="aSettings__input_input" id="timeBeforeMotionRearmInput" type="number" step="any" <%=Model.CanUpdate ? "" : "disabled"  %> name="ReportInterval" value="<%=Model.ReportInterval %>" />
...
<script type="text/javascript">
    ...
    { %>
        ...
        createSpinnerModal("reportNum", 'Check-in Time', "timeBeforeMotionRearmInput", reportInterval_array);
