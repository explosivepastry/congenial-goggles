    ...
    <%   if (TempData["CanCalibrate"].ToBool() && CalibrationCertificationValidUntil < MonnitSession.MakeLocal(DateTime.UtcNow))
     ...
     <div class="row sensorEditForm">
        <div class="col-12 col-md-3">
             <%: Html.TranslateTag("Sensor/ApplicationCustomization/app_082| Observed Sensor Reading: (From sensor)"," Observed Sensor Reading: (From sensor)")%>
        </div>
        <div class="col sensorEditFormInput">
              <input name="observed" id="observed" class="form-control" readonly="readonly" value="<%: Model.LastDataMessage != null ? Model.LastDataMessage.AppBase.PlotValue.ToDouble().ToString("0.###") : "" %>" />
                <%:Monnit.Pressure3000PSI.GetLabel(Model.SensorID) %>
        </div>
    </div>
     <div class="row sensorEditForm">
        <div class="col-12 col-md-3">
             <%: Html.TranslateTag("Sensor/ApplicationCustomization/app_082|Actual Input: (External reference value)","Actual Input: (External reference value)")%>
        </div>
        <div class="col sensorEditFormInput">
        ...
        <script>
            $(function ()
            {
                $('#actual').change(function ()
                {
                    ...
                    {
                    var maxCalibrate = <%: Pressure3000PSI.GetSavedValue(Model.SensorID)%>;
