...
<script type="text/javascript">
    var lowVal = <%:   (ZeroToTwoHundredVolts.GetLowValue(Model.SensorID).ToInt())%>;
    var highVal = <%:  (ZeroToTwoHundredVolts.GetHighValue(Model.SensorID).ToInt())%>;

    $(function () {
          <% if(Model.CanUpdate) { %>

        const arrayForSpinner = arrayBuilder(lowVal, highVal, 5);
