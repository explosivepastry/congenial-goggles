...
<%
    ...
    MultiStageThermostat Data = MultiStageThermostat.Deserialize(Model.FirmwareVersion, Model.LastDataMessage.Data);

    double stageMinTemp = isF ? 1d.ToFahrenheit() : 1d; // 1 celsius
    double stageMaxTemp = isF ? 10d.ToFahrenheit() : 10d; // 10 celsius
    ...
    double occupiedSetpoint = Math.Round(MultiStageThermostat.GetOccupiedSetPoint(Model), 0);
    double unoccupiedCoolingSetpoint = Math.Round(MultiStageThermostat.GetUnoccupiedCoolingSetpoint(Model), 0);
    double unoccupiedHeatingSetpoint = Math.Round(MultiStageThermostat.GetUnoccupiedHeatingSetpoint(Model), 0);

    if (isF)
    {
        occupiedSetpoint = Math.Round(occupiedSetpoint.ToFahrenheit(), 0);
        unoccupiedCoolingSetpoint = Math.Round(unoccupiedCoolingSetpoint.ToFahrenheit(), 0);
        unoccupiedHeatingSetpoint = Math.Round(unoccupiedHeatingSetpoint.ToFahrenheit(), 0);
    }

    double occupiedSetpointThumbPercentValue = (occupiedSetpoint - minTemp) / inputRange;
    occupiedSetpointThumbPercentValue = occupiedSetpointThumbPercentValue < 0 ? 0 : occupiedSetpointThumbPercentValue;
    double unoccupiedCoolingSetpointThumbPercentValue = (unoccupiedCoolingSetpoint - minTemp) / inputRange;
    unoccupiedCoolingSetpointThumbPercentValue = unoccupiedCoolingSetpointThumbPercentValue < 0 ? 0 : unoccupiedCoolingSetpointThumbPercentValue;
    double unoccupiedHeatingSetpointThumbPercentValue = (unoccupiedHeatingSetpoint - minTemp) / inputRange;
    unoccupiedHeatingSetpointThumbPercentValue = unoccupiedHeatingSetpointThumbPercentValue < 0 ? 0 : unoccupiedHeatingSetpointThumbPercentValue;

    int heatCoolMode = MultiStageThermostat.GetHeatCoolMode(Model);
    bool isFanOn = Data.Fan;
...
<%-- NOTES FOR THERMOSTAT
    ...
    🔺  .red-triangle-pointer - Red triangle that points to the current temp. This alters between .blue-triangle-pointer. This pointer spins based on the css property transform: rotate(90deg);
--%>
...
<div class="temp-reading_container">
    <div class="circle-temp_container">
        ...
        <div class="two-columns-temp">
            <div class="circle-align">
                <div id="adjustmentOnAndroid" style="white-space: nowrap; overflow: hidden; text-overflow: ellipsis;width: 100%; text-align:center" class="circle-temp-name" ><%=Model.SensorName%></div>
                <div id="adjustmentOnAndroidDate" style="text-align:center" class="circle-temp-date" >On <%=Model.LastDataMessage.MessageDate.OVToLocalDateShort() +" " + Model.LastDataMessage.MessageDate.OVToLocalTimeShort()%></div>
                ...
                <div class="circle-container">
                    ...
                    <div class="thermo-phase-container">
                        <div class="thermo-phase-btn">
                            ...
                            <%if (Data.HvacStatus == 1)//Heating
                                {%>
                            <div style="width: 25px">
                                <%=Html.GetThemedSVG("fire") %>
                            </div>
                            <%}
                                else if (Data.HvacStatus == 2)//Cooling
                                {%>
                            <div style="width: 25px"><%=Html.GetThemedSVG("snowflake") %></div>
                            <%} %>
                        </div>
                    </div>
            ...
            <div class="wrapit">
                ...
                <div class="slider-container">
                    <input class="rangeInput" id="occupiedSetpointRange" type="range" min="<%=minTemp %>" max="<%=maxTemp %>">
                    <input class="rangeInput" id="unoccupiedCoolingSetpointRange" type="range" min="<%=minTemp %>" max="<%=maxTemp %>">
                    <input class="rangeInput" id="unoccupiedHeatingSetpointRange" type="range" min="<%=minTemp %>" max="<%=maxTemp %>">

                    <svg id="slider" height="350" width="150" viewBox="-70 -10 200 300">
                        <path id="curve" stroke="gray" stroke-width="4" fill="none" d="" />
                        
                        <circle id="occupiedSetpointThumb" class="thumbElement aboveThumb" stroke="#fff" fill="#fff" r="15" />
                        <text id="occupiedSetpointThumbNum" class="thumbElement aboveThumbNum" x="61.09375" y="148.75" text-anchor="middle" alignment-baseline="central"><%=occupiedSetpoint %></text>
                        
                        <circle id="unoccupiedHeatingSetpointThumb" class="thumbElement aboveThumb inactiveThumb" stroke="#fff" fill="#fff" r="15" />
                        <text id="unoccupiedHeatingSetpointThumbNum" class="thumbElement aboveThumbNum inactiveThumb" x="61.09375" y="148.75" text-anchor="middle" alignment-baseline="central"><%=unoccupiedHeatingSetpoint %></text>
                        <circle id="unoccupiedCoolingSetpointThumb" class="thumbElement belowThumb inactiveThumb" stroke="#fff" fill="#fff" r="15" />
                        <text id="unoccupiedCoolingSetpointThumbNum" class="thumbElement belowThumbNum inactiveThumb" x="61.09375" y="148.75" text-anchor="middle" alignment-baseline="central"><%=unoccupiedCoolingSetpoint %></text>
                    </svg>
                   </div>
            </div>
        </div>
    ...
    <form action="/Overview/MultithermostatUiEdit/<%=Model.SensorID %>" method="post" class="multithermostatUiForm">
                ...
                <div class="select_wrap">
                    ...
                    <ul class="select_ul">
                        ...
                        <%
                            ...
                            if (systemType > 0)// Show heat for everything except "Not Defined (type 0)"
                            { %>
                        <li id="TempMode2">
                            <div class="option tog-1">
                                <div class="snow-fire-icons"><%:Html.GetThemedSVG("fire") %></div>
                                <div class="autoselect"><%:Html.TranslateTag("Heat") %></div>
                            </div>
                        </li>
                        <%}
                            if (systemType > 3)// Show cool and auto for everything except "Not Defined (type 0), 1 Stage Heat (Type 1), 2 Stage Heat (Type 2), 3 Stage Heat (Type 3), "
                            { %>

                        <li id="TempMode3">
                            <div class="option tog-2">
                                <div class="snow-fire-icons" style="width: 20px;"><%=Html.GetThemedSVG("snowflake") %></div>
                                <div class="autoselect"><%:Html.TranslateTag("Cool") %></div>
                            </div>
                        </li>
                        ...
                        <li id="TempMode1">
                            <div class="option ice">
                                <div class="auto-box">
                                    ...
                                    <div class="autoselect"><%:Html.TranslateTag("Auto") %></div>
                                </div>
                            </div>
                        </li>
                        <%} %>
                        <li id="TempMode0">
                            <div class="option ice">
                                <div><%=Html.GetThemedSVG("power-on-off") %></div>
                                <div class="autoselect"><%:Html.TranslateTag("Off") %></div>
                            </div>
                        </li>
                    </ul>
                </div>
                <%}%>
                    <input type="hidden" id="HeatCoolMode" name="HeatCoolMode" value="<%=heatCoolMode %>"/> 
            </div>
        ...
        <div id="occupied" class="selection">
                <div class="all-box-container">
                    <div class="thermo-box-container">
                        <div class="thermo-minus blue-icon general-btn" data-id="occupiedSetpoint"> 
                            <%=Html.GetThemedSVG("minus-circle") %>
                        </div>

                        <div class="thermo-box-below">
                            <div><%:Html.TranslateTag("Setpoint") %></div>
                            <input style="text-align:center" id="occupiedSetpointInput" name="OccupiedSetpoint" type="number" value="<%=occupiedSetpoint %>" min="<%=minTemp %>" max="<%=maxTemp %>" class="below-temp-set" />
                        </div>

                        <div class="thermo-plus blue-icon general-btn" data-id="occupiedSetpoint">
        ...
        <div id="unoccupied" class="selection" style="display: none">
                <div class="all-box-container">
                    ...
                    <div class="thermo-box-container">
                        <div class="thermo-minus blue-icon general-btn" data-id="unoccupiedCoolingSetpoint">
                            <%=Html.GetThemedSVG("minus-circle") %>
                        </div>

                        <div class="thermo-box-above">
                            <div><%:Html.TranslateTag("Turn Cooling on at:") %></div>
                            <input id="unoccupiedCoolingSetpointInput" name="UnoccupiedCoolingSetpoint" type="number" value="<%=unoccupiedCoolingSetpoint %>" min="<%=minTemp %>" max="<%=maxTemp %>" class="below-temp-set">
                        </div>
                        <div class="thermo-plus blue-icon general-btn" data-id="unoccupiedCoolingSetpoint">
                            <%=Html.GetThemedSVG("circle-add") %>
                        </div>
                    </div>

                      <div class="thermo-box-container">
                        <div class="thermo-minus blue-icon general-btn" data-id="unoccupiedHeatingSetpoint">
                            <%=Html.GetThemedSVG("minus-circle") %>
                        </div>

                        <div class="thermo-box-below">
                            <div><%:Html.TranslateTag("Turn Heating on at:") %></div>
                            <input id="unoccupiedHeatingSetpointInput" name="UnoccupiedHeatingSetpoint" type="number" value="<%=unoccupiedHeatingSetpoint %>" min="<%=minTemp %>" max="<%=maxTemp %>" class="below-temp-set">
                        </div>
                        <div class="thermo-plus blue-icon general-btn" data-id="unoccupiedHeatingSetpoint">
                            <%=Html.GetThemedSVG("circle-add") %>
                        </div>
                    </div>
                ...
                <div class="bottom-container" id="btmContainer">
                    <button type="submit" class="saveBtn btn btn-primary" id="MultistageSaveBtn"><%:Html.TranslateTag("SAVE") %></button>
                </div>
...
<script>
    ...
    const moveThumb = (e, isInput) => {
        ...
        let saveBtnElement = document.getElementById('MultistageSaveBtn');
    ...
    document.addEventListener("DOMContentLoaded", () => {
        ...
        const occupiedSetpointRangeEl = document.getElementById('occupiedSetpointRange');
        const occupiedSetpointThumbEl = document.getElementById('occupiedSetpointThumb');
        const occupiedSetpointThumbNumEl = document.getElementById('occupiedSetpointThumbNum');
        const occupiedSetpointInputEl = document.getElementById('occupiedSetpointInput');
        const occupiedSetpointPercent = Number('<%=occupiedSetpointThumbPercentValue%>');

        const unoccupiedCoolingSetpointRangeEl = document.getElementById('unoccupiedCoolingSetpointRange');
        const unoccupiedCoolingSetpointThumbEl = document.getElementById('unoccupiedCoolingSetpointThumb');
        const unoccupiedCoolingSetpointThumbNumEl = document.getElementById('unoccupiedCoolingSetpointThumbNum');
        const unoccupiedCoolingSetpointInputEl = document.getElementById('unoccupiedCoolingSetpointInput');
        const unoccupiedCoolingSetpointPercent = Number('<%=unoccupiedCoolingSetpointThumbPercentValue%>');

        const unoccupiedHeatingSetpointRangeEl = document.getElementById('unoccupiedHeatingSetpointRange');
        const unoccupiedHeatingSetpointThumbEl = document.getElementById('unoccupiedHeatingSetpointThumb');
        const unoccupiedHeatingSetpointThumbNumEl = document.getElementById('unoccupiedHeatingSetpointThumbNum');
        const unoccupiedHeatingSetpointInputEl = document.getElementById('unoccupiedHeatingSetpointInput');
        const unoccupiedHeatingSetpointPercent = Number('<%=unoccupiedHeatingSetpointThumbPercentValue%>');
        ...
        occupiedSetpointRangeEl.addEventListener('input', e => moveThumb(e, false));
        occupiedSetpointRangeEl.value = Number('<%=occupiedSetpoint%>');
        occupiedSetpointThumbEl.addEventListener('touchstart', e => thumbClick(e));
        occupiedSetpointThumbEl.addEventListener('click', e => thumbClick(e));
        occupiedSetpointThumbNumEl.addEventListener('touchstart', e => thumbClick(e));
        occupiedSetpointThumbNumEl.addEventListener('click', e => thumbClick(e));
        occupiedSetpointRangeEl.addEventListener('mouseup', e => thumbMouseout(e));
        occupiedSetpointRangeEl.addEventListener('touchend', e => thumbMouseout(e));
        occupiedSetpointInputEl.addEventListener('change', e => moveThumb(occupiedSetpointRangeEl, true));
        occupiedSetpointInputEl.addEventListener('blur', e => inputMouseout(e));

        unoccupiedCoolingSetpointRangeEl.addEventListener('input', e => moveThumb(e, false));
        unoccupiedCoolingSetpointRangeEl.value = Number('<%=unoccupiedCoolingSetpoint%>');
        unoccupiedCoolingSetpointThumbEl.addEventListener('touchstart', e => thumbClick(e));
        unoccupiedCoolingSetpointThumbEl.addEventListener('click', e => thumbClick(e));
        unoccupiedCoolingSetpointThumbNumEl.addEventListener('touchstart', e => thumbClick(e));
        unoccupiedCoolingSetpointThumbNumEl.addEventListener('click', e => thumbClick(e));
        unoccupiedCoolingSetpointRangeEl.addEventListener('mouseup', e => thumbMouseout(e));
        unoccupiedCoolingSetpointRangeEl.addEventListener('touchend', e => thumbMouseout(e));
        unoccupiedCoolingSetpointInputEl.addEventListener('change', e => moveThumb(unoccupiedCoolingSetpointRangeEl, true));
        unoccupiedCoolingSetpointInputEl.addEventListener('blur', e => inputMouseout(e));

        unoccupiedHeatingSetpointRangeEl.addEventListener('input', e => moveThumb(e, false));
        unoccupiedHeatingSetpointRangeEl.value = Number('<%=unoccupiedHeatingSetpoint%>');
        unoccupiedHeatingSetpointThumbEl.addEventListener('touchstart', e => thumbClick(e));
        unoccupiedHeatingSetpointThumbEl.addEventListener('click', e => thumbClick(e));

        unoccupiedHeatingSetpointThumbNumEl.addEventListener('touchstart', e => thumbClick(e));
        unoccupiedHeatingSetpointThumbNumEl.addEventListener('click', e => thumbClick(e));
        unoccupiedHeatingSetpointRangeEl.addEventListener('mouseup', e => thumbMouseout(e));
        unoccupiedHeatingSetpointRangeEl.addEventListener('touchend', e => thumbMouseout(e));
        unoccupiedHeatingSetpointInputEl.addEventListener('change', e => moveThumb(unoccupiedHeatingSetpointRangeEl, true));
        unoccupiedHeatingSetpointInputEl.addEventListener('blur', e => inputMouseout(e));
        ...
        const targetElements = document.querySelectorAll('#occupiedSetpointThumbNum, #unoccupiedHeatingSetpointThumbNum, #unoccupiedCoolingSetpointThumbNum');
        ...
        addEventListenerForClass('submit', 'multithermostatUiForm', function (e) {
            e.preventDefault();
    ...
    const saveButton = document.querySelector("#MultistageSaveBtn");
    ...
    const showOverlay = (userUpdated) => {
        ...
        console.log("is dirty on load/or save click?", sendingResponseToNetwork)
    ...
    function openSelect(selection) {
        ...
        // Thermo UI
        var rangeElement, elementStarterName;
        if (selection === 'unoccupied') {
            elementStarterName = selection + 'CoolingSetpoint';
            ...
            elementStarterName = selection + 'HeatingSetpoint';
            rangeElement = document.getElementById(elementStarterName + 'Range');
            ...
            if (selection === 'occupied') {
                elementStarterName = selection + 'Setpoint';
            }
            rangeElement = document.getElementById(elementStarterName + 'Range');
