...
<%
    ...
    StringBuilder InstantaneousDataVals = new StringBuilder();
    StringBuilder TWADataVals = new StringBuilder();


    foreach (DataMessage item in list)
    {
        CO_Meter Data = Monnit.CO_Meter.Deserialize(Model.Sensors[0].FirmwareVersion, item.Data);
        InstantaneousDataVals.Append("[new Date(" + Monnit.TimeZone.GetLocalTimeById(item.MessageDate, MonnitSession.CurrentCustomer.Account.TimeZoneID).ToEpochTime() + ")," + Data.CO_Instantaneous.ToString("#0.#") + "," + ((item.State & 2) == 2 ).ToInt() + ", 'Instantaneous' ,'" + item.DataMessageGUID +  "' ],");
        TWADataVals.Append("[new Date(" + Monnit.TimeZone.GetLocalTimeById(item.MessageDate, MonnitSession.CurrentCustomer.Account.TimeZoneID).ToEpochTime() + ")," + Data.CO_TWA.ToString("#0.#") + "," + ((item.State & 2) == 2 ).ToInt() + ", 'TWA' ,'" + item.DataMessageGUID +  "' ],");
    }  
%>
                ...
                series: [
                   {
                       ...
                       type: 'line',
                       showAllSymbol: true,
                       data: [<%=InstantaneousDataVals.ToString().TrimEnd(',')%>],
