...
<%
    ...
    bool isF = HumiditySHT25.IsFahrenheit(Model.Sensors[0].SensorID);
    ...
    double maxAware = (Model.Sensors[0].MaximumThreshold / 10.0);
    double minAware = (Model.Sensors[0].MinimumThreshold / 10.0);


    StringBuilder TemperatureDataVals = new StringBuilder();
    StringBuilder PressureDataVals = new StringBuilder();

    string label = DifferentialPressure.GetLabel(Model.Sensors[0].SensorID);
    ...
    {
        DifferentialPressure Data = Monnit.DifferentialPressure.Deserialize(Model.Sensors[0].FirmwareVersion, item.Data);
        ...
        PressureDataVals.Append("[new Date(" + Monnit.TimeZone.GetLocalTimeById(item.MessageDate, MonnitSession.CurrentCustomer.Account.TimeZoneID).ToEpochTime() + ")," + item.AppBase.PlotValue.ToDouble() + "," + ((item.State & 2) == 2).ToInt() + ", 'Pressure' ,'" + item.DataMessageGUID + "' ],");
    }
