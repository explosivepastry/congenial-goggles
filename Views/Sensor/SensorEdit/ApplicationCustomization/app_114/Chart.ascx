...
<%
    bool isF = AirSpeed.IsFahrenheit(Model.Sensors[0].SensorID);
    ...
    StringBuilder tempVals = new StringBuilder();
    bool showTemp = Monnit.AirSpeed.GetShowFullDataValue(Model.Sensors[0].SensorID);


    string label = AirSpeed.AbreviatedMesaurement(AirSpeed.GetUnits(Model.Sensors[0].SensorID));
    foreach (DataMessage item in list)
    {
        AirSpeed Data = (AirSpeed)MonnitApplicationBase.LoadMonnitApplication(Model.Sensors[0].FirmwareVersion, item.Data, Model.Sensors[0].ApplicationID, Model.Sensors[0].SensorID);
        dataVals.Append("[new Date(" + Monnit.TimeZone.GetLocalTimeById(item.MessageDate, MonnitSession.CurrentCustomer.Account.TimeZoneID).ToEpochTime() + ")," + item.AppBase.PlotValue.ToDouble().ToString("#0.###", System.Globalization.CultureInfo.InvariantCulture) + "," + ((item.State & 2) == 2).ToInt() + ",'" + item.DataMessageGUID + "'],");

        if (showTemp)
        {
            tempVals.Append("[new Date(" + Monnit.TimeZone.GetLocalTimeById(item.MessageDate, MonnitSession.CurrentCustomer.Account.TimeZoneID).ToEpochTime() + ")," + (isF ? Data.Temperature.ToFahrenheit() : Data.Temperature).ToDouble().ToString("#0.#") + "," + ((item.State & 2) == 2).ToInt() + ", 'Temperature' ,'" + item.DataMessageGUID + "' ],");
        }
    }
    ...
    {%>
                    {
                        ...
                        showAllSymbol: true,
                        yAxisIndex: 1,
                        data: [<%=tempVals.ToString().TrimEnd(',')%>],
                    }
                    <%}%>
