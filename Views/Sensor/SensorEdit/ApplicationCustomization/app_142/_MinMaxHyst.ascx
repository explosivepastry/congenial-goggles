...
<% 
    ...
    VibrationHysteresis = AdvancedVibration2.ConvertVibrationBasedonModeAndScale(Model,AdvancedVibration2.GetVibrationHysteresis(Model)).ToString();
    VibrationAwareThreshold = AdvancedVibration2.ConvertVibrationBasedonModeAndScale(Model,AdvancedVibration2.GetVibrationAwareThreshold(Model)).ToString();

    velocityScale = AdvancedVibration2.GetVelocityScale(Model.SensorID);
    accelerationScale = AdvancedVibration2.GetAccelerationScale(Model.SensorID);
    displacementScale = AdvancedVibration2.GetDisplacementScale(Model.SensorID);
%>
...
<div class="row sensorEditForm">
    <div class="col-12 col-md-3">
        <%: Html.TranslateTag("Sensor/ApplicationCustomization/app_111|Vibration Sensitivity Threshold", "Vibration Sensitivity Threshold")%> (g)
    </div>
    <div class="col sensorEditFormInput">
...
<div class="row sensorEditForm">
    ...
    <div class="col sensorEditFormInput">
        <select <%=Model.CanUpdate ? "" : "disabled" %> id="WindowFunction_Manual" name="WindowFunction_Manual" class="form-select">
            <option <%: WindowFunction == "0" ? "selected" : "" %> value="0">No Filter</option>
...
<div class="row sensorEditForm">
    ...
    <div class="col sensorEditFormInput">
        <select <%=Model.CanUpdate ? "" : "disabled" %> id="AccelerometerRange_Manual" name="AccelerometerRange_Manual" class="form-select">
            <option <%: AccelerometerRange == "0" ? "selected" : "" %> value="0">2 g</option>
            <option <%: AccelerometerRange == "1" ? "selected" : "" %> value="1">4 g</option>
            <option <%: AccelerometerRange == "2" ? "selected" : "" %> value="2">8 g</option>
            <option <%: AccelerometerRange == "3" ? "selected" : "" %> value="3">16 g</option>
        </select>
    </div>
</div>
...
<script type="text/javascript">
    ...
    var VibrationHystMax = (parseFloat($("#VibrationAwareThreshold_Manual").val()) * 0.5);;
    var accelerationLabel = '<%=accelerationScale%>';
    var velocityLabel = '<%=velocityScale%>';
    var displacementLabel = '<%=displacementScale%>';
    ...
    $(function () {
        var VAtMode = <%=VibrationMode%>
            setVATlabel(VAtMode);
              ...
              <% if (Model.CanUpdate)
    {%>
        //Settings for Minimum Sensitivity (g) - good beween edit field and number picker
        createSpinnerModal("VSTThreshNum", "g", "VibrationSensitivityThreshold_Manual", [1, 2], null, [".00", ".10", ".20", ".30", ".40", ".50", ".60", ".70", ".80", ".90"]);

        //maybe settings Number Picker for Measurement Interval (sec)
        createSpinnerModal("measIntNum", "Seconds", "MeasurementInterval_Manual", measureInt_array);

        /*Settings for Vibration Hysteresis (mm/s or mm/s^2) */
        const arrayForSpinner = arrayBuilder(0, VibrationHystMax, 1);
        createSpinnerModal("vibHystNum", label, "VibrationHysteresis_Manual", arrayForSpinner, null, [".00", ".10", ".20", ".30", ".40", ".50", ".60", ".70", ".80", ".90"]);

        //* --> Settings for Vibration Aware Trheshold (mm/s or mm/s^2) <-- */

        const arrayForSpinner1 = arrayBuilder(0, VatMax, 1);
        createSpinnerModal("vatNum", "Threshold", "VibrationAwareThreshold_Manual", arrayForSpinner1, null, [".00", ".10", ".20", ".30", ".40", ".50", ".60", ".70", ".80", ".90"]);
        ...
        $('#VibrationAwareThreshold_Manual').change(function () {
            if (isANumber(parseFloat($("#VibrationAwareThreshold_Manual").val()))) {
                if (Number($("#VibrationAwareThreshold_Manual").val()) < 0) {
                    $("#VibrationAwareThreshold_Manual").val(0);
                }
                if (Number($("#VibrationAwareThreshold_Manual").val()) > VatMax) {
                    $("#VibrationAwareThreshold_Manual").val(VatMax);
                }
            }
            else {
                $('#VibrationAwareThreshold_Manual').val(<%: VibrationAwareThreshold%>);
            }

            VibrationHystMax = parseInt(parseFloat($("#VibrationAwareThreshold_Manual").val()) * 0.5);
            $('#VibrationHysteresis_Manual').change();
        });
        ...
        $('#VibrationHysteresis_Manual').change(function () {
            if (isANumber($("#VibrationHysteresis_Manual").val())) {
                ...
                if ($("#VibrationHysteresis_Manual").val() > VibrationHystMax)
                    $("#VibrationHysteresis_Manual").val(VibrationHystMax);
            }
            else {
                $('#VibrationHysteresis_Manual').val(<%: VibrationHysteresis%>);
            }

            $('#vibHystNum').mobiscroll('option', {
                max: VibrationHystMax
            });
    ...
    function setVATSettings(mode) {
        switch (mode) {
            case 0:
                switch (velocityLabel) {
                    case "in/s peak":
                        label = 'in/s peak';
                        VatMax = (655.35 * 35.92).toFixed(4);
                        VATStep = .01;
                        break;
                    case "in/s rms":
                        label = 'in/s rms';
                        VatMax = (655.35 * 25.4).toFixed(3);
                        VATStep = .01;
                        break;
                    case "mm/s":
                    default:
            ...
            case 2:
            case 1:
                switch (accelerationLabel) {
                    case "g":
                        label = 'g';
                        VatMax = (65535 / 9806.65).toFixed(3);
                        VATStep = .1;
                        break;
                    case "in/s^2":
                        label = 'in/s^2';
                        VatMax = (65535 / 25.4).toFixed(1);
                        VATStep = 10;
                        break;
                    case "mm/s^2":
                    default:
            ...
            case 4:
                switch (displacementLabel) {
                    case "mil p-p":
                        label = 'mil p-p';
                        VatMax = (655.35 * 39.37).toFixed(3);
                        VATStep = .01;
                        break;
                    case "in p-p":
                        label = 'in p-p';
