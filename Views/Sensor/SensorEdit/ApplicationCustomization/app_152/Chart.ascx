...
<%
    ...
    StringBuilder LevelDataVals = new StringBuilder();

    foreach (DataMessage item in list)
    {
        LN2Level Data = LN2Level.Deserialize(Model.Sensors[0].FirmwareVersion, item.Data);
        LevelDataVals.Append("[new Date(" + Monnit.TimeZone.GetLocalTimeById(item.MessageDate, MonnitSession.CurrentCustomer.Account.TimeZoneID).ToEpochTime() + ")," + Data.Level.ToString("#0.##") + "," + ((item.State & 2) == 2).ToInt() + ", 'Level','" + item.DataMessageGUID + "' ],");
    }
                ...
                series: [
                    {
                        ...
                        showAllSymbol: true,
                        yAxisIndex: 1,
                        data: [<%=LevelDataVals.ToString().TrimEnd(',')%>],
