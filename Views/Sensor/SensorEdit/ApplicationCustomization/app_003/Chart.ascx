...
<%
    ...
    string OneValue = DryContact.GetOneValue(Model.Sensors[0].SensorID);
    string ZeroValue = DryContact.GetZeroValue(Model.Sensors[0].SensorID);

    foreach (DataMessage item in list)
    {
        dataVals.Append("[new Date(" + Monnit.TimeZone.GetLocalTimeById(item.MessageDate, MonnitSession.CurrentCustomer.Account.TimeZoneID).ToEpochTime() + ")," + item.Data.ToBool().ToInt() + "," + ((item.State & 2) == 2).ToInt() + ",'" + item.DataMessageGUID +  "'],");
    }
