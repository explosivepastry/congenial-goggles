...
<%
    // 074 = 0-10V Measure PreAgg Chart

    StringBuilder minVals = new StringBuilder();
    StringBuilder maxVals = new StringBuilder();
    StringBuilder avgVals = new StringBuilder();
    double LowValue = ZeroToTenVolts.GetLowValue(Model.sensor.SensorID);
    double HighValue = ZeroToTenVolts.GetHighValue(Model.sensor.SensorID);
    string label = ZeroToTenVolts.GetLabel(Model.sensor.SensorID);

    foreach (PreAggregatedData item in Model.PreAggregateList)
    {
        minVals.Append("[new Date(" + item.Date.ToUniversalTime().ToEpochTime() + ")," + item.Min.LinearInterpolation(0, LowValue, 10, HighValue).ToString("#0.###", System.Globalization.CultureInfo.InvariantCulture) + "],");
        maxVals.Append("[new Date(" + item.Date.ToUniversalTime().ToEpochTime() + ")," + item.Max.LinearInterpolation(0, LowValue, 10, HighValue).ToString("#0.###", System.Globalization.CultureInfo.InvariantCulture) + "],");
        avgVals.Append("[new Date(" + item.Date.ToUniversalTime().ToEpochTime() + ")," + item.Avg.LinearInterpolation(0, LowValue, 10, HighValue).ToString("#0.###", System.Globalization.CultureInfo.InvariantCulture) + "],");
    }
