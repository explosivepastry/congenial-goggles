...
<%
    ...
    bool isF = QuartzdynePressure.IsFahrenheit(Model.Sensors[0].SensorID);
    ...
    StringBuilder PressureDataVals = new StringBuilder();



    foreach (DataMessage item in list)
    {
        QuartzdynePressure Data = Monnit.QuartzdynePressure.Deserialize(Model.Sensors[0].FirmwareVersion, item.Data);
        TemperatureDataVals.Append("[new Date(" + Monnit.TimeZone.GetLocalTimeById(item.MessageDate, MonnitSession.CurrentCustomer.Account.TimeZoneID).ToEpochTime() + ")," + (isF ? Data.Temperature.ToFahrenheit().To7Digits() : Data.Temperature.To7Digits()).ToString() + "," + ((item.State & 2) == 2 ).ToInt() + ", 'Temperature','" + item.DataMessageGUID +  "' ],");
        PressureDataVals.Append("[new Date(" + Monnit.TimeZone.GetLocalTimeById(item.MessageDate, MonnitSession.CurrentCustomer.Account.TimeZoneID).ToEpochTime() + ")," + Data.Pressure.To7Digits().ToString() + "," + ((item.State & 2) == 2 ).ToInt() + ", 'Pressure' ,'" + item.DataMessageGUID +  "' ],");
    }
                ...
                series: [
                   {
                       ...
                       type: 'line',
                       showAllSymbol: true,
                       data: [<%=PressureDataVals.ToString().TrimEnd(',')%>],
