...
<%
    // 099 = Resistance Delta PreAgg Chart

    StringBuilder minVals = new StringBuilder();
    StringBuilder maxVals = new StringBuilder();
    StringBuilder avgVals = new StringBuilder();
    double LowValue = ResistanceDelta.GetLowValue(Model.sensor.SensorID);
    double HighValue = ResistanceDelta.GetHighValue(Model.sensor.SensorID);
    string label = ResistanceDelta.GetLabel(Model.sensor.SensorID);

    foreach (PreAggregatedData item in Model.PreAggregateList)
    {
        minVals.Append("[new Date(" + item.Date.ToUniversalTime().ToEpochTime() + ")," + item.Min.LinearInterpolation(0, LowValue, 250000, HighValue).ToString("#0.###", System.Globalization.CultureInfo.InvariantCulture) + "],");
        maxVals.Append("[new Date(" + item.Date.ToUniversalTime().ToEpochTime() + ")," + item.Max.LinearInterpolation(0, LowValue, 250000, HighValue).ToString("#0.###", System.Globalization.CultureInfo.InvariantCulture) + "],");
        avgVals.Append("[new Date(" + item.Date.ToUniversalTime().ToEpochTime() + ")," + item.Avg.LinearInterpolation(0, LowValue, 250000, HighValue).ToString("#0.###", System.Globalization.CultureInfo.InvariantCulture) + "],");
    }
