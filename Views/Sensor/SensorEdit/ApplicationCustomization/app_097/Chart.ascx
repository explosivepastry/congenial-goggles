...
<%
    ...
    bool isF = Thermostat.IsFahrenheit(sens.SensorID);
    ...
    double maxAware = Thermostat.GetOccupiedCoolingThreshold(sens);
    double minAware = Thermostat.GetOccupiedHeatingThreshold(sens);

    if (isF)
    {
        maxAware = maxAware.ToFahrenheit();
        minAware = minAware.ToFahrenheit();
    }

    foreach (DataMessage item in list)
    {
        Thermostat Data = Monnit.Thermostat.Deserialize(sens.FirmwareVersion, item.Data);
        ...
        HeaterDataVals.Append("[new Date(" + Monnit.TimeZone.GetLocalTimeById(item.MessageDate, MonnitSession.CurrentCustomer.Account.TimeZoneID).ToEpochTime() + ")," + ((Data.Heater.ToInt() > 0) ? 1 : 0) + "," + ((item.State & 2) == 2 ).ToInt() + ", 'Heater' ,'" + item.DataMessageGUID +  "' ],");
        CoolerDataVals.Append("[new Date(" + Monnit.TimeZone.GetLocalTimeById(item.MessageDate, MonnitSession.CurrentCustomer.Account.TimeZoneID).ToEpochTime() + ")," + ((Data.Cooler.ToInt() > 0) ? 1 : 0) + "," + ((item.State & 2) == 2 ).ToInt() + ", 'Cooler' ,'" + item.DataMessageGUID +  "' ],");
    }    
%>
                        ...
                        data: [{
                            ...
                            label: {
                                normal: {
                                    color: 'black',
                                    position: 'middle',
                                    formatter: 'Occupied Heat Threshold'
                                }
                            },
                            yAxis: '<%=minAware%>'
                        }, {
                            label: {
                                normal: {
                                    color: 'black',
                                    position: 'middle',
                                    formatter: 'Occupied Cool Threshold'
                                }
                            },
                            yAxis: '<%=maxAware%>'
                            }]
