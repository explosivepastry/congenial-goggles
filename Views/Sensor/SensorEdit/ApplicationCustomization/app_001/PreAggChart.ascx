...
<%
    // 001 = Analog Voltage PreAgg Chart

    StringBuilder minVals = new StringBuilder();
    StringBuilder maxVals = new StringBuilder();
    StringBuilder avgVals = new StringBuilder();
    double LowValue = Analog.GetLowValue(Model.sensor.SensorID);
    double HighValue = Analog.GetHighValue(Model.sensor.SensorID);
    string label = Analog.GetLabel(Model.sensor.SensorID);

    foreach (PreAggregatedData item in Model.PreAggregateList)
    {
        minVals.Append("[new Date(" + item.Date.ToUniversalTime().ToEpochTime() + ")," + item.Min.LinearInterpolation(0, LowValue, 1.2, HighValue).ToString("#0.###", System.Globalization.CultureInfo.InvariantCulture) + "],");
        maxVals.Append("[new Date(" + item.Date.ToUniversalTime().ToEpochTime() + ")," + item.Max.LinearInterpolation(0, LowValue, 1.2, HighValue).ToString("#0.###", System.Globalization.CultureInfo.InvariantCulture) + "],");
        avgVals.Append("[new Date(" + item.Date.ToUniversalTime().ToEpochTime() + ")," + item.Avg.LinearInterpolation(0, LowValue, 1.2, HighValue).ToString("#0.###", System.Globalization.CultureInfo.InvariantCulture) + "],");
    }
