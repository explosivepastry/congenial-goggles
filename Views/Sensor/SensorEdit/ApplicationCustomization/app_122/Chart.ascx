...
<%
    ...
    string label = VoltageMeter500VAC.GetLabel(Model.Sensors[0].SensorID);
    foreach (DataMessage item in list)
    {
        dataVals.Append("[new Date(" + Monnit.TimeZone.GetLocalTimeById(item.MessageDate, MonnitSession.CurrentCustomer.Account.TimeZoneID).ToEpochTime() + ")," + item.AppBase.PlotValue.ToDouble().ToString("#0.###") + "," + ((item.State & 2) == 2).ToInt() + ",'" + item.DataMessageGUID + "'],");
    }
