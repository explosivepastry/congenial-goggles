...
<%
    ...
    {
        Gas_H2S Data = Monnit.Gas_H2S.Deserialize(Model.Sensors[0].FirmwareVersion, item.Data);
        InstantaneousDataVals.Append("[new Date(" + Monnit.TimeZone.GetLocalTimeById(item.MessageDate, MonnitSession.CurrentCustomer.Account.TimeZoneID).ToEpochTime() + ")," + Data.Instantaneous.ToString("#0.#") + "," + ((item.State & 2) == 2 ).ToInt() + ", 'Instantaneous','" + item.DataMessageGUID +  "' ],");
        TWADataVals.Append("[new Date(" + Monnit.TimeZone.GetLocalTimeById(item.MessageDate, MonnitSession.CurrentCustomer.Account.TimeZoneID).ToEpochTime() + ")," + Data.TWA.ToString("#0.#") + "," + ((item.State & 2) == 2 ).ToInt() + ", 'TWA','" + item.DataMessageGUID +  "' ],");
    }  
%>
