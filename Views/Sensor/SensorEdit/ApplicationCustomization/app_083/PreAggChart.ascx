...
<%
    // 082 = Pressure Custom PreAgg Chart

    StringBuilder minVals = new StringBuilder();
    StringBuilder maxVals = new StringBuilder();
    StringBuilder avgVals = new StringBuilder();
    double LowValue = PressureNPSI.GetLowValue(Model.sensor.SensorID);
    double HighValue = PressureNPSI.GetHighValue(Model.sensor.SensorID);
    string label = PressureNPSI.GetLabel(Model.sensor.SensorID);
    ...
    double savedValue = 0;
    SensorAttribute sv = SensorAttribute.LoadBySensorID(Model.sensor.SensorID).Where(c => c.Name == "SavedValue").FirstOrDefault();
    if (sv != null)
    {
        savedValue = sa.Value.ToDouble();
    }
