...
<%
    ...
    double maxAware = (Model.Sensors[0].MaximumThreshold / 100.0);
    double minAware = (Model.Sensors[0].MinimumThreshold / 100.0);
    ...
    {
        LightSensor Data = Monnit.LightSensor.Deserialize(Model.Sensors[0].FirmwareVersion, item.Data);
        LuxDataVals.Append("[new Date(" + Monnit.TimeZone.GetLocalTimeById(item.MessageDate, MonnitSession.CurrentCustomer.Account.TimeZoneID).ToEpochTime() + ")," + Data.PlotValue + "," + ((item.State & 2) == 2).ToInt() + ", 'Lux' ,'" + item.DataMessageGUID + "' ],");
        LightDataVals.Append("[new Date(" + Monnit.TimeZone.GetLocalTimeById(item.MessageDate, MonnitSession.CurrentCustomer.Account.TimeZoneID).ToEpochTime() + ")," + ((Data.Light.ToInt() > 0) ? 1 : 0) + "," + ((item.State & 2) == 2).ToInt() + ", 'Light' ,'" + item.DataMessageGUID + "' ],");
    }
%>
                    ...
                    formatter: function (params) {
                        ...
                        if (params.value[3] == 'Lux') {
                            ...
                            return "Date: " + DatePreferenceFormat(params.value[0], '<%=(MonnitSession.CurrentCustomer.Preferences["Date Format"])%>') + ' ' + TimePreferenceFormat(params.value[0], '<%=(MonnitSession.CurrentCustomer.Preferences["Time Format"]).Replace(" tt","") %>') + '<br/>' +
                                'Value: ' + params.value[1] + ' lux' + '<br/>' +
                                'Status: ' + (params.value[2] == 1 ? 'Aware' : 'Not Aware');
                        } else {

                            return "Date: " + DatePreferenceFormat(params.value[0], '<%=(MonnitSession.CurrentCustomer.Preferences["Date Format"])%>') + ' ' + TimePreferenceFormat(params.value[0], '<%=(MonnitSession.CurrentCustomer.Preferences["Time Format"]).Replace(" tt","") %>') + '<br/>' +
                                'Value: ' + (params.value[1] == 1 ? 'Light' : 'No Light') + '<br/>' +
                                'Status: ' + (params.value[2] == 1 ? 'Aware' : 'Not Aware');
                        }
                    }
                },
            ...
            data: [
                {
                    label: {
                        normal: {
                            color: 'black',
                            position: 'middle',
                            formatter: 'Minimum Threshold- ' + '<%=minAware%>'
                        }
                    },
                    yAxis: '<%=minAware%>'
                },
                {
                    label: {
                        normal: {
                            color: 'black',
                            position: 'middle',
                            formatter: 'Maximum Threshold- ' + '<%=maxAware%>'
                        }
                    },
                    yAxis: '<%=maxAware%>'
                                }
