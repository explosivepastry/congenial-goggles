...
<%
    ...
    string label = ThreePhase20AmpMeter.GetLabel(Model.Sensors[0].SensorID);


    foreach (DataMessage item in list)
    {
        ThreePhase20AmpMeter Data = Monnit.ThreePhase20AmpMeter.Deserialize(Model.Sensors[0].FirmwareVersion, item.Data);
