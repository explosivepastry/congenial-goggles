...
<form action="/Overview/SensorScale/<%:Model.SensorID %>" id="SensorScale_<%:Model.SensorID %>" method="post">
    ...
    <%
        string volts = ThreePhase20AmpMeter.GetVoltValue(Model.SensorID).ToString();
        string Hours = ThreePhase20AmpMeter.GetLabel(Model.SensorID);
        double estimatedPowerFactor = ThreePhase20AmpMeter.GetPowerFactor(Model.SensorID);
        int phaseSelection = ThreePhase20AmpMeter.GetSelectedCurrentReading(Model.SensorID);
        int useEstimatedPower = ThreePhase20AmpMeter.GetUseEstimatedPower(Model.SensorID).ToBool().ToInt();
         %>
    ...
    <div class="row sensorEditForm voltRow" >
        ...
        <div class="col sensorEditFormInput">
            ...
            <div style="display: none;"><%: Html.TextBox("APorEP_Manual",useEstimatedPower, (Dictionary<string,object>)ViewData["HtmlAttributes"])%></div>
        </div>
    </div>
    <div class="clear"></div>
    <br />
