...
<%
    bool AwareOnMotion = PIRHumidity.GetAwareOnMotion(Model).ToBool();
    double reArmTime = (PIRHumidity.GetRearmTime(Model) / 60.0);
    int sensitivity = PIRHumidity.GetPIRSensitivity(Model);
    bool isF = PIRHumidity.IsFahrenheit(Model.SensorID);
%>
<p class="useAwareState">Enter Aware State Motion</p>
<div class="row sensorEditForm">
    <div class="col-12 col-md-3 ">
        <%: Html.TranslateTag("Sensor/ApplicationCustomization/app_138|On Motion")%>
    </div>
    <div class="col sensorEditFormInput">
        ...
        <div style="display: none;">
            <input id="ReportImmediatelyOn" name="ReportImmediatelyOn" type="text" value="<%=AwareOnMotion.ToInt() %>"></div>
    </div>
</div>
...
<div class="row sensorEditForm">
    <div class="col-12 col-md-3">
        <%: Html.TranslateTag("Sensor/ApplicationCustomization/app_138|Re-Arm Time")%>  (<%: Html.TranslateTag("Minutes") %>)
    </div>
    <div class="col sensorEditFormInput">
...
<div class="row sensorEditForm">
    <div class="col-12 col-md-3">
        <%: Html.TranslateTag("Sensor/ApplicationCustomization/app_138|Sensitivity","Sensitivity")%>
    </div>
    <div class="col sensorEditFormInput">
...
<script type="text/javascript">
    ...
    $(document).ready(function () {
                <% if (Model.CanUpdate)
    { %>
        const max = Number($('#ActiveStateInterval').val());
        const arrayForSpinner = arrayBuilder(1, max, 1);
        createSpinnerModal("reArmNum", "Minutes", "ActiveStateInterval", arrayForSpinner);
