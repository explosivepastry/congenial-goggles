...
<%
    ...
    bool isF = PIRHumidity.IsFahrenheit(Model.Sensors[0].SensorID);
    ...
    //Sec Chart
    StringBuilder trueMotionDataVal = new StringBuilder();
    StringBuilder falseMotionDataVal = new StringBuilder();
    string TrueMotionVal = "Motion";
    string FalseMotionVal = "No Motion";

    foreach (DataMessage item in list)
    {
        PIRHumidity Data = (PIRHumidity)MonnitApplicationBase.LoadMonnitApplication(Model.Sensors[0].FirmwareVersion, item.Data, Model.Sensors[0].ApplicationID, Model.Sensors[0].SensorID);
        ...
        //Sec Chart
       trueMotionDataVal.Append("[new Date(" + Monnit.TimeZone.GetLocalTimeById(item.MessageDate, MonnitSession.CurrentCustomer.Account.TimeZoneID).ToEpochTime() + ")," + ((item.AppBase.PlotValue.ToInt() > 0) ? 1 : 0) + "," + ((item.State & 2) == 2).ToInt() + ", 'Motion' ,'" + item.DataMessageGUID +  "' ],");
       falseMotionDataVal.Append("[new Date(" + Monnit.TimeZone.GetLocalTimeById(item.MessageDate, MonnitSession.CurrentCustomer.Account.TimeZoneID).ToEpochTime() + ")," + ((item.AppBase.PlotValue.ToInt() > 0) ? 1 : 0) + "," + ((item.State & 2) == 2).ToInt() + ", 'Motion' ,'" + item.DataMessageGUID +  "' ],");
            ...
            baseOption: {
                tooltip: {
                    trigger: 'item',
                        ...
                        var date = new Date(params.value[0]);
                        return "Date: " + DatePreferenceFormat(params.value[0], '<%=(MonnitSession.CurrentCustomer.Preferences["Date Format"])%>') + ' ' + TimePreferenceFormat(params.value[0], '<%=(MonnitSession.CurrentCustomer.Preferences["Time Format"]).Replace(" tt","") %>') + '<br/>' +
                            'Value: ' + (params.value[1] == 1 ? '<%=TrueMotionVal%>' : '<%=FalseMotionVal%>') + '<br/>' +
                            'Status: ' + (params.value[2] == 1 ? 'Aware' : 'Not Aware');
                            }
                },
                ...
                yAxis: [{
                    type: 'category',
                    data: ['<%=FalseMotionVal%>', '<%=TrueMotionVal%>']
                }],
                ...
                series: [
                    ...
                    type: 'line',
                        step: 'end',
                        showAllSymbol: true,
                    data: [<%=trueMotionDataVal.ToString().TrimEnd(',')%>]
                },
                {
                    name: '<%=FalseMotionVal%>',
                    type: 'line',
                    step: 'end',
                    showAllSymbol: true,
                        data: [<%=falseMotionDataVal.ToString().TrimEnd(',')%>]
                    }
                ]
            },
