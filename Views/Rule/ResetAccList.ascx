        ...
        <div class="choose-task">
            <div class="control-unit_container w-100">
                ...
                <div class="command-thermo_container " style="border-radius: 20px!important;">
                    ...
                    <div class="control-unit_grid" id="deviceList">
                        ...
                        <% 
                            ...
                            {%>
                        <div filter="<%: device.SensorName.ToLower() %>" class="deviceHolder" id="resetAccDevice_<%=device.SensorID %>">
                            <%=Html.Partial("ResetAccDetails",device) %>
                        </div>
                        <%} %>
                    </div>
                ...
                <div class="ccu-button">
                    ...
                    <button type="button" class=" user-dets btn btn-primary" id="saveResetAccTextBtn"><%: Html.TranslateTag("Save")%></button>
                </div>
            </div>
...
<script type="text/javascript">
    ...
    function toggleResetAccRelayState(divID) {
        var obj = $('#' + divID);
        if (obj.prop('checked')) {
    ...
    function toggleResetAccDiv(deviceID, relayNumber, doHide) {
        if (doHide) {
    ...
    //function toggleResetAccDiv(deviceID, relayNumber, doHide) {
    //    $('#caretClose_' + deviceID + '_' + relayNumber).css("display", doHide ? "none" : "block");
    //    $('#caretOpen_' + deviceID + '_' + relayNumber).css("display", doHide ? "block" : "none");
    //    $('#editRelay' + relayNumber + '_' + deviceID).css("height", doHide ? "57px" : "auto");
    //}
