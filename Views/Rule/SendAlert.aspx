...
<asp:Content ID="Content2" ContentPlaceHolderID="MainContent" runat="server">
    ...
    <%
        ...
        List<NotificationRecipient> notificationRecipients = MonnitSession.NotificationRecipientsInProgress.ToList();
        ...
        bool shouldShowAdvanced = MonnitSession.NotificationInProgress.VoiceText.Length > 0 || MonnitSession.NotificationInProgress.SMSText.Length > 0;
    ...
    <div class="column-on-sm-scr" style="display: flex; gap: 1rem;">
        <%--  USERS CARD   --%>
            ...
            <div>
                ...
                <div class="card_container__body">
                    ...
                    <% if (userGroups.Count > 0)
                        { %>
                    <div class="panel panel-heading" style="margin-bottom: 19px;">
                ...
                <div style="display: flex; justify-content: center;">
                    <div class="msg_card1AB width-adjustment" style="z-index: 1">
                        <div class="txt-heading">
                            <input type="text" id="subject" name="subject" placeholder="Subject..." value="<%= MonnitSession.NotificationInProgress.Subject != null ? MonnitSession.NotificationInProgress.Subject : "" %>" class="msg-input user-dets" />
                        ...
                        <textarea id="alertMessage" style="margin: 0 !important" class="text-box-msg user-dets send-text-area-size" placeholder="<%:Html.TranslateTag("Rule/SendNotification|Message...")%>" name="alertMessage"><%=MonnitSession.NotificationInProgress.NotificationText.Length.ToInt() > 0 ? MonnitSession.NotificationInProgress.NotificationText  : "Device: {Name}({ID}) Reading: {Reading}" %></textarea>
                        ...
                        <div id="advancedAlertMessages">
                            ...
                            <textarea style="margin: 0 !important" class="text-box-msg user-dets send-text-area-size" placeholder="Enter SMS/Push message..." id="alertMessageSMS" name="alertMessageSMS" maxlength="160"><%= MonnitSession.NotificationInProgress.SMSText.Length.ToInt() > 0 ? MonnitSession.NotificationInProgress.SMSText : "" %></textarea>
                            <div class="msg-save">
                                <div class="d-flex" style="align-items: center; gap: 1rem;">
                                    <a href="#" class="overridePreview sendPreviewSMS" data-type="Text" data-bs-toggle="modal" data-bs-target=".previewOverrideSMS"><strong style="color: #5A738E;"><%= Html.TranslateTag("SMS/Push Preview") %></strong></a>
                                </div>
                            </div>

                            <div style="font-weight: 600; margin: 0.5rem 0">Voice Specific Message</div>
                            <textarea style="margin: 0 !important" class="text-box-msg user-dets send-text-area-size" placeholder="Enter Voice message..." id="alertMessageVoice" name="alertMessageVoice"><%= MonnitSession.NotificationInProgress.VoiceText.Length.ToInt() > 0 ? MonnitSession.NotificationInProgress.VoiceText : "" %></textarea>
                            <div class="msg-save">
                                <div class="d-flex" style="align-items: center; gap: 1rem;">
    ...
    <script type="text/javascript">
        ...
        let clearInputFields = (idsOfInputsArray) => {
        ...
        async function saveNotificationSettings(Voicevalues, PostString) {
            try {
                const response = await new Promise((resolve, reject) => {
                    $.post(`/Rule/Save${PostString}NotificationSettings`, Voicevalues, function (data) {
                        if (data !== 'Success') {
                            resolve(data);
                        } else {
                            resolve('Success');
        ...
        async function postNotificationRecipients(valuesToPost) {
            try {
                const response = await new Promise((resolve, reject) => {
                    $.post(`/Rule/SaveAllRecipients/`, valuesToPost, function (data) {
                        if (data !== 'Success') {
                            resolve(data);
                        } else {
                            resolve('Success');
                ...
                if (response !== 'Success, saved recipients') {
                    failedServerResponse = true;
        ...
        async function saveForm() {
            ...
            //Make sure the user submits an notification message of somekind.
            ...
            await saveNotificationSettings(values, 'Email');

            const smsValues = {
                SMSText: $('#alertMessageSMS').val()
            }

            await saveNotificationSettings(smsValues, 'Text');
            ...
            await saveNotificationSettings(Pushvalues, 'Push');

            await saveNotificationSettings(voiceValues, 'Voice');
            ...
            if (failedServerResponse) {
                ...
                setTimeout(() => {
                    window.location = '/Rule/ChooseTask/';
                    $('#saving').hide();
                    $('#saveAlert').show();
                }, 1000)
            }
    ...
    <% } %>
        ...
        async function saveRecipients() {
            ...
            await postNotificationRecipients({ emailValuesToSubmit, smsValuesToSubmit, voiceValuesToSubmit, groupValuesToSubmit, pushValuesToSubmit  });
        };

        function RemoveNotifyTask(eNotiType) {
            if (confirm("Are you sure you want to delete this alert?")) {
                $.post("/Rule/RemoveNotifyTask/", function (data) {
                    if (data == "Success") {
