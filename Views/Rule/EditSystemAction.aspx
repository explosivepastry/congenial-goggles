...
<asp:Content ID="Content2" ContentPlaceHolderID="MainContent" runat="server">
    ...
    <% 
        List<ExternalDataSubscription> externalDataSubscriptions = ExternalDataSubscription.LoadAllByAccountID(Model.AccountID);
    ...
    <div class="csa_container">
                ...
                <form id="ActionForm" style="max-width: 315px; margin-top: 10px;">
                    ...
                    <div class="  section system-action-select">
                        ...
                        <div class="col-md-4 col-12">
                            <select name="action" id="Action" class="form-select user-dets" style="width: 250px;">
                    ...
                    <div class=" section system-action-select">
                        ...
                        <div class="col-md-4 col-12">
                            <select name="DelayMinutes" class="form-select user-dets" style="width: 250px;">
                        ...
                        <div class="col-md-6 col-12">
                            <select id="TargetRule" name="TargetRule" class="form-select user-dets select-action" style="width: 250px;">
                                ...
                                <% foreach (Notification item in ViewBag.NotificationList)
                                    {%>
                                <option <%=item.NotificationID == Model.NotificationID ? "selected='selected'": "" %> value="<%=item.NotificationID%>"><%=item.Name %></option>
                                <%} %>
                            </select>
                        </div>
                    ...
                    <div class="section system-action-select" style="display: none;">
                        <div class="col-12  sub-text">
                            <label for="TargetAccumulator"><%: Html.TranslateTag("Rule/CreateSystemAction|Target Device")%></label>
                        </div>
                        <div class="col-md-6 col-12">
        ...
        <%
            List<NotificationRecipient> systemActionList = (List<NotificationRecipient>)ViewBag.SystemActionList;
        %>
            <div class="action-place">
                <div id="systemActionHolder" class="csa-list" <%=systemActionList.Count == 0 ? "style=\"display:none;\"" : "" %>>
                    <div class="" id="systemActionList">
                        <%=Html.Partial("CreateSystemActionList", systemActionList) %>
                    </div>
                </div>
                <div class="done-btn">
                    <a id="go-back" class="btn btn-primary user-dets"><%= Html.TranslateTag("Done")%> 
                    </a>
                </div>
    ...
    <div class="modal fade systemActionHelp" style="z-index: 2000!important;" tabindex="-1" role="dialog" aria-hidden="true">
        <div class="modal-dialog modal-dialog-centered">
            <%=Html.Partial("_SystemActionHelpModalBody") %>
        </div>
    </div>

    <script type="text/javascript">
        document.getElementById("go-back").onclick = function () {
            window.location.href = "/Rule/ChooseTaskToEdit/<%=Model.NotificationID%>";
        }
        var failedAction = '<%: Html.TranslateTag("Rule/CreateSystemAction|Failed to set action","Failed to set action")%>';

        $(document).ready(function () {
            $('#AddButton').click(function () {
                var body = $('#ActionForm').serialize();
                var href = "/Rule/EditPageSetSystemAction/<%=Model.NotificationID%>";
        ...
        function loadSystemActionList() {
            ...
            $.get("/Rule/EditSystemActionList/<%=Model.NotificationID%>", function (data) {
        ...
        function removeSystemAction(notificationType, properties, delay, executeID) {
            ...
            $.post("/Rule/EditPageSystemActionDelete/<%=Model.NotificationID%>", { notificationType: notificationType, properties: properties, delay: delay, executeID: executeID }, function (data) {
                if (data == "Success") {
                    loadSystemActionList();
                } else {
                    console.log(data);
