<%@ Page Title="" Language="C#" MasterPageFile="~/Views/Shared/Default.Master" Inherits="System.Web.Mvc.ViewPage<Notification>" %>
...
<asp:Content ID="Content2" ContentPlaceHolderID="MainContent" runat="server">
    <% List<NotificationRecipientData> userList = NotificationRecipientData.SearchPotentialRecipient(MonnitSession.CurrentCustomer.CustomerID, Model.AccountID, "");

        List<NotificationRecipient> assignedRecipients = ViewBag.notificationRecipients as List<NotificationRecipient>;
    %>

    <%long NotiID = Model.NotificationID; %>
    <%:Html.Partial("~/Views/Rule/Header.ascx",Model) %>

    <div class="msg_container">
        <div class="msg-title" style="width: auto;">
            <div class="card_container__top__title">
                <%: Html.TranslateTag("Send Email", "Send Email")%>
            </div>
            <div class="col-xs-2" id="newRefresh" style="padding-left: 0px;">
                <div style="float: right; margin-bottom: 5px;">
                    <div class="btn-group" style="height: 45px;">
                        <a onclick="removeNotifyTask('<%=eNotificationType.Email.ToInt() %>');" style="cursor: pointer;" title="<%=Html.TranslateTag("Remove") %>"><%=Html.GetThemedSVG("delete") %></a>
                    </div>
                </div>
            </div>
            ...
            <div class=" msg_card1">
                <div class="msg-top">
                    ...
                    <div onclick="$('#myModal').modal('toggle');" class="user-inputs-box" style="cursor: pointer; display: flex;">
                        ...
                        <div>
                            <div class="user-stamp" id="emailrecipients"><%=Html.Partial("_SendToNamesList", assignedRecipients) %></div>
                ...
                <div class="msg-sub">
                    ...
                    <input type="text" id="subject" name="subject" required="required"placeholder="Subject" value="<%=Model.Subject %>" class="msg-input user-dets">
                    ...
                    <div class="msg-sub-title">
                        ...
                        <div class="merge-me">
                            <%if (MonnitSession.AccountCan("text_override"))
                                {%>
                            <a data-bs-toggle="modal" data-bs-target=".overrideValues" title="<%: Html.TranslateTag("Rule/SendNotification|Merge Fields")%>" style="cursor: pointer;">
                                <p class="merge-btn"><%= Html.TranslateTag("Merge Fields") %></p>
                            </a>
                            <%} %>
                        </div>
                ...
                <textarea id="editor"><%=Model.NotificationText%></textarea>

                <a href="#" class="msg-preview overridePreview" data-type="Email" data-bs-toggle="modal" data-bs-target=".previewOverride"><strong><%= Html.TranslateTag("Message Preview") %></strong></a>
                <div class="msg-save">
                    <div id="saveNotificationBtnSaveMsg"></div>
                    <button type="button" class="btn btn-primary user-dets" id="saveNotificationBtn"><%: Html.TranslateTag("Save")%></button>
                </div>
            </div>
    ...
    <div class="modal fade " id="myModal">
        <div class="modal-dialog modal-dialog-scrollable">
            <div class="modal-content modal_container">
                ...
                <div class="modal-footer">
                    <a href="#" onclick="saveRecipients();" data-bs-dismiss="modal" class="btn btn-primary btn-outline-dark user-dets"><%= Html.TranslateTag("Save") %></a>
                </div>
            </div>
        </div>
    </div>
    ...
    <script>
        ...
        $(function () {
            ...
            $('.overridePreview').click(function (e) {
                e.preventDefault();
                var msg = sunObjEditor.getContents();
                var obj = $('#previewOverrideBody');
                obj.html("content/css/myloader.ascx");
                msg = msg.replace('{Subject}', $('#subject').val());
                $.post('/Rule/EditPageMsgPreview/<%=NotiID%>', { msg: msg }, function (data) {
                    obj.html(data);
                });
            });
            ...
            $('#saveNotificationBtn').click(function () {
                ...
                values["type"] = "<%=eNotificationType.Email%>";
                values["subject"] = $('#subject').val();
                values["emailMsg"] = sunObjEditor.getContents();

                var recipients = $('.notifyUser.selected');
                if (recipients.length == 0) {
                    toastBuilder("No Recipient Selected")
                    //$('#saveNotificationBtnSaveMsg').append('<div class="errorMsg">No recipient selected</div>');
                    //$('#saveNotificationBtnSaveMsg').show();

                } else {
                    $.post('/Rule/EditEmailNotificationSettings/<%=NotiID%>', values, function (data) {
                        if (data == "Success") {
/*                            $('#saveNotificationBtnSaveMsg').html('<div class="successMsg">' + successString + '</div>').show();*/
                            ...
                            for (var i = 0; i < errors.length; i++) {
                                ...
                                // var errorObjID = errorData[0];
                                //var objToFind = errorObjID.length > 0 ? $('#' + errorObjID) : undefined;
                                //if (objToFind == undefined || objToFind.length == 0) {
                                //    $('#saveNotificationBtnSaveMsg').append(`<div class="errorMsg">${errorMsg}</div>`);
                                //    $('#saveNotificationBtnSaveMsg').show();
                                //}
                                //else {
                                //    objToFind.before(`<div class="errorMsg">${errorMsg}</div>`);
                                //}
                            }
        ...
        function removeNotifyTask(eNotiType) {
            ...
            $.post("/Rule/EditPageRemoveNotifyTask/<%=NotiID%>", { NotiType: eNotiType }, function (data) {
                if (data == "Success") {
                    window.location.href = "/Rule/ChooseTaskToEdit/<%=NotiID%>";
                } else {
        ...
        function saveRecipients() {
            ...
            var values = new Object();
            values["type"] = "<%=eNotificationType.Email.ToInt()%>";
            var recipients = $('.notifyUser.selected');
            var recipientString = '';
            ...
            $.post('/Rule/EditPageSaveRecipients/<%=NotiID%>', values, function (data) {
