<%@ Control Language="C#" Inherits="System.Web.Mvc.ViewUserControl<List<CustomerGroupLink>>" %>

<div class="mh400">
    <%
        List<Customer> allCustomersInAccount = Customer.LoadAllByAccount(MonnitSession.CurrentCustomer.AccountID);
        ...
        Model.Count(m => m.CustomerID == cust.CustomerID && m.DelayMinutes >= 0))
            .ThenBy(cust => cust.FirstName);

        foreach (Customer cust in orderedCustomers)
        {
            ...
            CustomerGroupLink emailLink = Model.Where(m => { return m.CustomerID == cust.CustomerID && m.NotificationType == eNotificationType.Email; }).FirstOrDefault();
            emailActive = emailLink != null;
            if (emailActive) emailDelay = emailLink.DelayMinutes <= int.MinValue ? 0 : emailLink.DelayMinutes;

            CustomerGroupLink textLink = Model.Where(m => { return m.CustomerID == cust.CustomerID && m.NotificationType == eNotificationType.SMS; }).FirstOrDefault();
            textActive = textLink != null;
            if (textActive) textDelay = textLink.DelayMinutes <= int.MinValue ? 0 : textLink.DelayMinutes;

            CustomerGroupLink voiceLink = Model.Where(m => { return m.CustomerID == cust.CustomerID && m.NotificationType == eNotificationType.Phone; }).FirstOrDefault();
            voiceActive = voiceLink != null;
            if (voiceActive) voiceDelay = voiceLink.DelayMinutes <= int.MinValue ? 0 : voiceLink.DelayMinutes;

            CustomerGroupLink pushLink = Model.Where(m => { return m.CustomerID == cust.CustomerID && m.NotificationType == eNotificationType.Push_Message; }).FirstOrDefault();
            pushActive = pushLink != null;
            if (pushActive) pushDelay = pushLink.DelayMinutes <= int.MinValue ? 0 : pushLink.DelayMinutes;
    ...
    <div style="font-size: 1.4em" class="flexAB recipientCardAB" data-recipientname="<%=cust.FullName %>" data-recipientid="<%=cust.CustomerID %>">
        ...
        <div class="col-2AB shrink-col-2-mobile" style="display: flex; flex-direction: column">
            <%if (pushEnabledUserList.Any(PWAuser=> PWAuser.CustomerID == cust.CustomerID))
                {%>
            ...
            function handleToggleNotificationMethod(customerId, deliveryMethodString) {
                ...
                if (parseInt(currentValue) >= 0) {
                    ...
                    switch (deliveryMethodString) {
                        case "Push":
                            changePushSpinnerHighlightedValue(false, customerId, 0);
                            break;
                        case "Voice":
                    ...
                    switch (deliveryMethodString) {
                        case "Push":
                            changePushSpinnerHighlightedValue(false, customerId, 1);
                            break;
                        case "Voice":
