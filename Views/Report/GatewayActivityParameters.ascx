<%@ Control Language="C#" Inherits="System.Web.Mvc.ViewUserControl<iMonnit.Models.InboundPacketReportModel>" %>
    ...
    <% using (Html.BeginForm()) {%>
        ...
        %>
            <div class="editor-label">
                <%: Html.LabelFor(model => model.CSNetID) %>
            </div>
            <div class="editor-field">
                <%: Html.TextBoxFor(model => model.CSNetID) %>
            </div>
            <div class="editor-error">
                <%: Html.ValidationMessageFor(model => model.CSNetID)%>
            </div>
...
<script type="text/javascript">
    $(document).ready(function () {
        ...
        $('#CSNetID').keydown(function (event) {
            if (event.keyCode == 13) {
