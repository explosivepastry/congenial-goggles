...
<%--
<div class="daterangePickerMenu" style="display: none; position: absolute;">
    <div style="float: right;">
        <%: Html.TranslateTag("Overview/DateRangePickerMenu|Quick Pick","Quick Pick")%>: 
        <select id="historySelect" style="width: 150px;">
            <option value="null">&nbsp;</option>
            <option value="0"><%: Html.TranslateTag("Overview/DateRangePickerMenu|Today","Today")%></option>
            <option value="-7"><%: Html.TranslateTag("Overview/DateRangePickerMenu|Last 7 days","Last 7 days")%></option>
            <option value="-30"><%: Html.TranslateTag("Overview/DateRangePickerMenu|Last 30 days","Last 30 days")%></option>
        </select>
        <input type="button" class="bluebutton" value="<%: Html.TranslateTag("Overview/DateRangePickerMenu|Go","Go")%>" style="margin: -1px 0px 0px 10px;" />
    </div>
...
<script>
    ...
    function setServerDate(input) {
        if (input != null) {
            $.get('/Home/Set' + input.attr('name') + '?date=' + input.val(), function (data) {
                if (data != "Success") {
                    console.log(data);
    ...
    function setDateRangePicker() {
        $('.historyFromDate, .historyToDate').click(function (e) {
            ...
            $('.daterangePickerMenu').css({ top: pos.top + 35, left: pos.left - 100 }).show();
