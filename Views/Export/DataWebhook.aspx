        ...
        <div class="col-12">
            ...
            <div class="text-end">
                <% if (Model != null && Model.ExternalDataSubscriptionID > 0)
                    { %>
                <button type="button" value="<%: Html.TranslateTag("Export/DataWebhook|Configure","Configure")%>" class="btn btn-primary" onclick="configure('<%=Model.eExternalDataSubscriptionType %>')">
                    <%: Html.TranslateTag("Export/DataWebhook|Configure","Configure")%>
                </button>
                <% }
                    else
                    { %>
                <button type="button" value="<%: Html.TranslateTag("Export/DataWebhook|Create","Create")%>" class="btn btn-primary" onclick="chooseType()">
                    <%: Html.TranslateTag("Export/DataWebhook|Create", "Create")%>
                </button>
                <%}%>
            </div>
