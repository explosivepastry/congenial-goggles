...
<input type="hidden" name="creditClassification" value="<%=eCreditClassification.GatewayUnlockGps.ToInt() %>" />
...
<div class="row" style="margin: 0px 0px 15px 5px;">
    <table class="table table-hover" style="background-color: white; margin: 0px; padding: 0px;">
        ...
        <tbody>
            <%foreach (Credit creditgroup in Credit.LoadAvailable(Model.AccountID,eCreditClassification.GatewayUnlockGps))
                {%>
            <tr>
                ...
                <td style="padding: 0px; padding-top: 6px;">
                    <%if (MonnitSession.IsCurrentCustomerMonnitSuperAdmin)
                        {%>
                        <a title="<%: Html.TranslateTag("Retail/GatewayUnlockGps|Remove Credits","Remove Credits")%>" href="/Retail/RemoveCredits/<%:creditgroup.CreditID %>" onclick="removeCredits(this); return false;">
            ...
            <%} %>
                <tr style="background-color: #e3e3e3; ">
                <th style="color:black;"><%: Html.TranslateTag("Retail/GatewayUnlockGps|Total","Total")%></th>
    ...
    $(function () {
        ...
        $('#submitCreditSettings').click(function () {
            ...
            $.post("/Retail/CreditSetting/<%:Model.CreditSettingID%>", creditform, function (data) {
                if (data == "Success") {
                    window.location.href = "/Retail/GatewayUnlockGps/<%=Model.AccountID%>";
                }
                else {
