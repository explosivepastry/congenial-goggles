...
<div class="row" style="margin: 0px 0px 15px 5px;">
    <table class="table table-hover" style="background-color: white; margin: 0px; padding: 0px;">
        ...
        <tbody>
            <%  List<Credit> credits = Credit.LoadAvailable(Model.Account.AccountID, eCreditClassification.GatewayUnlockGps);
                int Total = 0;
                foreach(Credit creditgroup in credits)
                {
                    Total += creditgroup.RemainingCredits;
                    %>
                    <tr id="gatewayUnlockGps_<%:creditgroup.CreditID %>">
        ...
        <script>
            $(function () {
                ...
                for (var i in itemArray) {
                    ...
                    $('#gatewayUnlockGps_' + itemID).css('border-style', 'solid').addClass("border-3 border-success");
                }
            });
        </script>
    <%}
