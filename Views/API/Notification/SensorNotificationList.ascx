...
<div class="methodDiv">
    <b>Method: SensorNotificationList</b><br />
    Returns all notifications of the specified Sensor<br />
    ...
    <table>
        <tr>
            <td>sensorID:</td>
            <td>Long</td>
            <td>Brings back a list of all notifications of a specific sensor</td>
        </tr>
    </table>
    
    
    <h4>Example</h4>
<%--    <a href="/xml/SensorNotificationList/Z3Vlc3Q6cGFzc3dvcmQ=?sensorID=101" target="_blank">https://<%:Request.Url.Host %>/xml/SensorNotificationList/Z3Vlc3Q6cGFzc3dvcmQ=?sensorID=101</a>--%>

<input type="button" id="btn_TryAPI_SensorNotificationList" class="btn btn-primary btn-sm" value="Try this API" />
	<script>
		$(function () {
            $('#btn_TryAPI_SensorNotificationList').click(function () {
				var json =
				{
					"auth": true,					
                    "api": "SensorNotificationList",
					"params": [
                        { "name": "sensorID", "type": "Long", "description": "Brings back a list of all notifications of a specific sensor", "optional": false }
					]
				};								
				APITest(json);
			});
