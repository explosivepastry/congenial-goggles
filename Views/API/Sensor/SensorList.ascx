...
<div class="methodDiv">
    ...
    <h4>Example</h4>
   <%-- <a href="/xml/SensorList/Z3Vlc3Q6cGFzc3dvcmQ=?name=Room3&applicationID=2" target="_blank">https://<%:Request.Url.Host %>/xml/SensorList/Z3Vlc3Q6cGFzc3dvcmQ=?name=Room3&amp;applicationID=2</a>
    <br />
    Only sensors that contain "Room3" in the name are returned
    <br />
    Only sensors with application id 2 (temperature) are returned --%>
    <input type="button" id="btn_TryAPI_SensorList" class="btn btn-primary btn-sm" value="Try this API" />

	<script>
		$(function () {
			$('#btn_TryAPI_SensorList').click(function () {
				var json =
				{
					"auth": true,					
					"api": "SensorList",
